...
#error "Unknown TARGET definition"
   ...
   status =
       PyConfig_SetString(&config, &config.executable, std::wstring(python_home.begin(), python_home.end()).c_str());
   ...
   // PyConfig_SetString(&config, &config.pythonpath_env, std::wstring(scripts_path.begin(),
   // scripts_path.end()).c_str());
...
void HooksPy::run_python_file_with_argv(const std::string &filename, const std::string &argv_str)
{
   ...
   // argv_str could never contain any spaces because of the configuration parser. Argvs are pased through argv in
   // run-sniper anyways.
