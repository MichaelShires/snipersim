...
static PyTypeObject statsGetterType = {
    .ob_base = PyVarObject_HEAD_INIT(NULL, 0).tp_name = "statsGetter",
...
static UInt64 statsCallback(String objectName, UInt32 index, String metricName, UInt64 _pFunc)
{
   ...
   PyObject *pResult =
       HooksPy::callPythonFunction(pFunc, Py_BuildValue("(sls)", objectName.c_str(), index, metricName.c_str()));
...
static PyObject *registerStats(PyObject *self, PyObject *args)
{
   ...
   Sim()->getStatsManager()->registerMetric(
       new StatsMetricCallback(objectName, index, metricName, (StatsCallback)statsCallback, (UInt64)pFunc));
...
static PyObject *writeMarker(PyObject *self, PyObject *args)
{
   ...
   Sim()->getStatsManager()->logMarker(Sim()->getClockSkewMinimizationServer()->getGlobalTime(), core_id, thread_id,
...
static PyMethodDef PyStatsMethods[] = {
    ...
    {"register", registerStats, METH_VARARGS,
     "Register callback that defines statistics value for (objectName, index, metricName)."},
    {"register_per_thread", registerPerThread, METH_VARARGS,
     "Add a per-thread statistic (perthreadName) based on a named statistic (objectName, metricName)."},
...
static PyModuleDef PyStatsModule = {PyModuleDef_HEAD_INIT, "sim_stats", "", -1, PyStatsMethods, NULL, NULL, NULL, NULL};
