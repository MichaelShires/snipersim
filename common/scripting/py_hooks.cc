...
/* Notes on the PyGILState, we only need to take a lock on the Global Interpreter lock, when executing the function from
 * a c-thread. All the other callback functions, called by the python scripts, which are assumed to have the GIL
 * already, so we don't need to explictly take it anymore.
 */
...
static SInt64 hookCallbackMagicMarkerType(UInt64 pFunc, UInt64 _argument)
{
   ...
   PyGILState_STATE state = PyGILState_Ensure();
   PyObject *pResult =
       HooksPy::callPythonFunction((PyObject *)pFunc, Py_BuildValue("(iiKKs)", argument->thread_id, argument->core_id,
...
static SInt64 hookCallbackThreadCreateType(UInt64 pFunc, UInt64 _argument)
{
   ...
   PyObject *pResult = HooksPy::callPythonFunction(
       (PyObject *)pFunc, Py_BuildValue("(ii)", argument->thread_id, argument->creator_thread_id));
   SInt64 result = hookCallbackResult(pResult);
   PyGILState_Release(state);
   check_and_abort();
   return result;
...
static SInt64 hookCallbackThreadTimeType(UInt64 pFunc, UInt64 _argument)
{
   ...
   PyGILState_STATE state = PyGILState_Ensure();
   PyObject *pResult =
       HooksPy::callPythonFunction((PyObject *)pFunc, Py_BuildValue("(iL)", argument->thread_id, time.getFS()));
   SInt64 result = hookCallbackResult(pResult);
   PyGILState_Release(state);
   check_and_abort();
   return result;
...
static SInt64 hookCallbackThreadStallType(UInt64 pFunc, UInt64 _argument)
{
   ...
   PyObject *pResult = HooksPy::callPythonFunction(
       (PyObject *)pFunc,
       Py_BuildValue("(isL)", argument->thread_id, ThreadManager::stall_type_names[argument->reason], time.getFS()));
   SInt64 result = hookCallbackResult(pResult);
   PyGILState_Release(state);
   check_and_abort();
   return result;
...
static SInt64 hookCallbackThreadResumeType(UInt64 pFunc, UInt64 _argument)
{
   ...
   PyGILState_STATE state = PyGILState_Ensure();
   PyObject *pResult = HooksPy::callPythonFunction(
       (PyObject *)pFunc, Py_BuildValue("(iiL)", argument->thread_id, argument->thread_by, time.getFS()));
   SInt64 result = hookCallbackResult(pResult);
   PyGILState_Release(state);
   check_and_abort();
   return result;
...
static SInt64 hookCallbackThreadMigrateType(UInt64 pFunc, UInt64 _argument)
{
   ...
   PyGILState_STATE state = PyGILState_Ensure();
   PyObject *pResult = HooksPy::callPythonFunction(
       (PyObject *)pFunc, Py_BuildValue("(iiL)", argument->thread_id, argument->core_id, time.getFS()));
   SInt64 result = hookCallbackResult(pResult);
   PyGILState_Release(state);
   check_and_abort();
   return result;
...
static SInt64 hookCallbackSyscallEnter(UInt64 pFunc, UInt64 _argument)
{
   ...
   PyObject *pResult = HooksPy::callPythonFunction(
       (PyObject *)pFunc,
       Py_BuildValue("(iiLi(llllll))", argument->thread_id, argument->core_id, time.getFS(), argument->syscall_number,
                     argument->args.arg0, argument->args.arg1, argument->args.arg2, argument->args.arg3,
                     argument->args.arg4, argument->args.arg5));
   SInt64 result = hookCallbackResult(pResult);
   PyGILState_Release(state);
   check_and_abort();
   return result;
...
static SInt64 hookCallbackSyscallExit(UInt64 pFunc, UInt64 _argument)
{
   ...
   PyGILState_STATE state = PyGILState_Ensure();
   PyObject *pResult = HooksPy::callPythonFunction(
       (PyObject *)pFunc, Py_BuildValue("(iiLiO)", argument->thread_id, argument->core_id, time.getFS(),
...
static PyModuleDef PyHooksModule = {PyModuleDef_HEAD_INIT, "sim_hooks", "", -1, PyHooksMethods, NULL, NULL, NULL, NULL};
