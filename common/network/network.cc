...
void Network::netPullFromTransport()
{
   do {
      ...
      LOG_PRINT("Pull packet : type %i, from %i, time %s", (SInt32)packet.type, packet.sender,
                itostr(packet.time).c_str());
      assert(0 <= packet.sender && packet.sender < _numMod);
      LOG_ASSERT_ERROR(0 <= packet.type && packet.type < NUM_PACKET_TYPES, "Packet type: %d not between 0 and %d",
                       packet.type, NUM_PACKET_TYPES);
      ...
      if (packet.receiver != _core->getId()) {
         ...
         LOG_PRINT("Forwarding packet : type %i, from %i, to %i, core_id %i, time %s.", (SInt32)packet.type,
                   packet.sender, packet.receiver, _core->getId(), itostr(packet.time).c_str());
      ...
      if (callback != NULL) {
         LOG_PRINT("Executing callback on packet : type %i, from %i, to %i, core_id %i, time %s", (SInt32)packet.type,
                   packet.sender, packet.receiver, _core->getId(), itostr(packet.time).c_str());
         assert(0 <= packet.sender && packet.sender < _numMod);
      ...
      // synchronous I/O support
         ...
         LOG_PRINT("Enqueuing packet : type %i, from %i, to %i, core_id %i, time %s.", (SInt32)packet.type,
                   packet.sender, packet.receiver, _core->getId(), itostr(packet.time).c_str());
...
SInt32 Network::netSend(NetPacket &packet)
{
   ...
   for (UInt32 i = 0; i < hopVec.size(); i++) {
      LOG_PRINT("Send packet : type %i, from %i, to %i, next_hop %i, core_id %i, time %s", (SInt32)packet.type,
                packet.sender, hopVec[i].final_dest, hopVec[i].next_dest, _core->getId(),
                itostr(hopVec[i].time).c_str());
      // LOG_ASSERT_ERROR(hopVec[i].time >= packet.time, "hopVec[%d].time(%llu) < packet.time(%llu)", i, hopVec[i].time,
      // packet.time);
...
class NetRecvIterator
{
 public:
   ...
   NetRecvIterator(const std::vector<SInt32> &v) : _mode(SENDER_VECTOR), _senders(&v), _i(0)
   {
   }
   NetRecvIterator(const std::vector<PacketType> &v) : _mode(TYPE_VECTOR), _types(&v), _i(0)
   {
   }
...
NetPacket Network::netRecv(const NetMatch &match, UInt64 timeout_ns)
{
   ...
   NetRecvIterator sender = match.senders.empty() ? NetRecvIterator(_numMod) : NetRecvIterator(match.senders);

   NetRecvIterator type =
       match.types.empty() ? NetRecvIterator((UInt32)NUM_PACKET_TYPES) : NetRecvIterator(match.types);

   LOG_ASSERT_ERROR(_core && _core->getPerformanceModel(), "Core and/or performance model not initialized.");
...
NetPacket::NetPacket()
    : start_time(SubsecondTime::Zero()), time(SubsecondTime::Zero()), queue_delay(SubsecondTime::Zero()),
      type(INVALID_PACKET_TYPE), sender(INVALID_CORE_ID), receiver(INVALID_CORE_ID), length(0), data(0)
{
}

NetPacket::NetPacket(SubsecondTime t, PacketType ty, SInt32 s, SInt32 r, UInt32 l, const void *d)
    : start_time(SubsecondTime::Zero()), time(t), queue_delay(SubsecondTime::Zero()), type(ty), sender(s), receiver(r),
      length(l), data(d)
{
}
