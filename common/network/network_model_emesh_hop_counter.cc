...
NetworkModelEMeshHopCounter::NetworkModelEMeshHopCounter(Network *net, EStaticNetwork net_type)
    : NetworkModel(net, net_type), _hopLatency(NULL, 0), _enabled(false), _num_packets(0), _num_bytes(0),
      _total_latency(SubsecondTime::Zero())
{
   ...
   try {
      _linkBandwidth = ComponentBandwidthPerCycle(Sim()->getDvfsManager()->getCoreDomain(net->getCore()->getId()),
                                                  Sim()->getCfg()->getInt("network/emesh_hop_counter/link_bandwidth"));
      _hopLatency = ComponentLatency(Sim()->getDvfsManager()->getCoreDomain(net->getCore()->getId()),
...
void NetworkModelEMeshHopCounter::computePosition(core_id_t core, SInt32 &x, SInt32 &y)
{
   x = core % _meshWidth;
   y = core / _meshWidth;
}
...
SubsecondTime NetworkModelEMeshHopCounter::computeSerializationLatency(UInt32 pkt_length)
{
