...
NetworkModel::NetworkModel(Network *network, EStaticNetwork net_type)
    : _network(network), m_collect_traffic_matrix(Sim()->getCfg()->getBool("network/collect_traffic_matrix"))
{
...
std::pair<bool, SInt32> NetworkModel::computeCoreCountConstraints(UInt32 network_type, SInt32 core_count)
{
   switch (network_type) {
   case NETWORK_MAGIC:
   case NETWORK_EMESH_HOP_COUNTER:
