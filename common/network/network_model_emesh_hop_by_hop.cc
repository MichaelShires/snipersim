...
const char *output_direction_names[] = {"up", "down", "left", "right", "---", "self", "peer", "destination"};
static_assert(NetworkModelEMeshHopByHop::MAX_OUTPUT_DIRECTIONS ==
                  sizeof(output_direction_names) / sizeof(output_direction_names[0]),
...
{
   LOG_ASSERT_ERROR(direction < NetworkModelEMeshHopByHop::MAX_OUTPUT_DIRECTIONS, "Invalid output direction %d",
                    direction);
...
NetworkModelEMeshHopByHop::NetworkModelEMeshHopByHop(Network *net, EStaticNetwork net_type)
    : NetworkModel(net, net_type), m_enabled(false), m_total_bytes_sent(0), m_total_packets_sent(0),
      m_total_bytes_received(0), m_total_packets_received(0), m_total_contention_delay(SubsecondTime::Zero()),
      m_total_packet_latency(SubsecondTime::Zero()), m_fake_node(false), m_core_id(getNetwork()->getCore()->getId()),
      // Placeholders.  These values will be overwritten in a derived class.
      m_link_bandwidth(Simulator::getSingleton() ? Sim()->getDvfsManager()->getCoreDomain(m_core_id) : NULL, 0),
      m_hop_latency(Simulator::getSingleton() ? Sim()->getDvfsManager()->getCoreDomain(m_core_id) : NULL, 0)
{
   // Get the Link Bandwidth, Hop Latency and if it has broadcast tree mechanism
   if (Simulator::getSingleton()) {
      try {
         // Link Bandwidth is specified in bits/clock_cycle
         m_link_bandwidth =
             ComponentBandwidthPerCycle(Sim()->getDvfsManager()->getCoreDomain(m_core_id),
                                        Sim()->getCfg()->getInt("network/emesh_hop_by_hop/link_bandwidth"));
         // Hop Latency is specified in cycles
         m_hop_latency = ComponentLatency(Sim()->getDvfsManager()->getCoreDomain(m_core_id),
      ...
      m_dimensions = 2;
      m_wrap_around = false;
      m_queue_model_enabled = false;
      m_queue_model_type = "history_list";
   ...
   if (Simulator::getSingleton())
      ...
      // Skip queue models for standalone testing unless needed
      m_injection_port_queue_model = NULL;
      m_ejection_port_queue_model = NULL;
      for (int i = 0; i < NUM_OUTPUT_DIRECTIONS; ++i)
         m_queue_models[i] = NULL;
   }
}
...
void NetworkModelEMeshHopByHop::createQueueModels(String name)
{
   ...
   m_injection_port_queue_model =
       QueueModel::create(name + ".link-in", m_core_id, m_queue_model_type, min_processing_time);
   m_ejection_port_queue_model =
       QueueModel::create(name + ".link-out", m_core_id, m_queue_model_type, min_processing_time);
}

void NetworkModelEMeshHopByHop::routePacket(const NetPacket &pkt, std::vector<Hop> &nextHops)
{
   ...
   if (pkt.receiver == NetPacket::BROADCAST) {
      if (m_fake_node) {
         ...
         if (cy == sy) {
            if (cx >= sx)
               addHop(RIGHT, NetPacket::BROADCAST, computeCoreId(cx + 1, cy), curr_time, pkt_length, nextHops,
                      requester);
            if (cx <= sx)
               addHop(LEFT, NetPacket::BROADCAST, computeCoreId(cx - 1, cy), curr_time, pkt_length, nextHops,
                      requester);
            if (cx == sx)
         ...
         LOG_ASSERT_ERROR(pkt.sender == m_core_id,
                          "BROADCAST message to be sent at (%i), original sender(%i), Tree not enabled", m_core_id,
                          pkt.sender);
...
void NetworkModelEMeshHopByHop::processReceivedPacket(NetPacket &pkt)
{
   ...
   SubsecondTime contention_delay =
       packet_latency - (computeDistance(pkt.sender, m_core_id) * m_hop_latency.getLatency());
...
void NetworkModelEMeshHopByHop::addHop(OutputDirection direction, core_id_t final_dest, core_id_t next_dest,
...
SInt32 NetworkModelEMeshHopByHop::computeDistance(core_id_t sender, core_id_t receiver)
{
   SInt32 sx, sy, dx, dy;
   ...
   if (m_wrap_around)
      return std::min(abs(sx - dx), m_mesh_width - abs(sx - dx)) + std::min(abs(sy - dy), m_mesh_height - abs(sy - dy));
   else
      return abs(sx - dx) + abs(sy - dy);
}

void NetworkModelEMeshHopByHop::computePosition(core_id_t core_id, SInt32 &x, SInt32 &y)
{
...
SubsecondTime NetworkModelEMeshHopByHop::computeLatency(OutputDirection direction, SubsecondTime pkt_time,
                                                        UInt32 pkt_length, core_id_t requester,
                                                        subsecond_time_t *queue_delay_stats)
{
   LOG_ASSERT_ERROR(!m_fake_node, "Cannot computeLatency on a fake network node");

   LOG_ASSERT_ERROR((direction >= 0) && (direction < NUM_OUTPUT_DIRECTIONS), "Invalid Direction(%u)", direction);
...
SubsecondTime NetworkModelEMeshHopByHop::computeInjectionPortQueueDelay(core_id_t pkt_receiver, SubsecondTime pkt_time,
                                                                        UInt32 pkt_length)
{
...
SubsecondTime NetworkModelEMeshHopByHop::computeEjectionPortQueueDelay(SubsecondTime pkt_time, UInt32 pkt_length)
{
...
SubsecondTime NetworkModelEMeshHopByHop::computeProcessingTime(UInt32 pkt_length)
{
...
SInt32 NetworkModelEMeshHopByHop::getNextDest(SInt32 final_dest, OutputDirection &direction)
{
...
void NetworkModelEMeshHopByHop::enable()
{
   m_enabled = true;
}

void NetworkModelEMeshHopByHop::disable()
{
   m_enabled = false;
}
...
void NetworkModelEMeshHopByHop::computeMeshDimensions(SInt32 &mesh_width, SInt32 &mesh_height)
{
   ...
   if (size == "") {
      ...
      default:
         LOG_PRINT_ERROR(
             "Invalid value %d for dimensions, only 1 (line/ring) and 2 (mesh/torus) are currently supported",
             dimensions);
      }

      LOG_ASSERT_ERROR(
          core_count == (concentration * mesh_height * mesh_width),
          "Cannot build a mesh with %d cores (concentration %d), increase NumApplicationCores to %d for a %d x %d mesh "
          "or configure network/emesh_hop_by_hop/size=WIDTH:HEIGHT to manually specify mesh dimensions",
          core_count, concentration, concentration * mesh_width * mesh_height, mesh_width, mesh_height);
   }
   else {
      ...
      LOG_ASSERT_ERROR(core_count == (concentration * mesh_height * mesh_width),
                       "Invalid mesh size %s for %d cores (concentration %d): %d x %d (x %d) == %d != %d", size.c_str(),
                       core_count, concentration, mesh_width, mesh_height, concentration,
                       concentration * mesh_width * mesh_height, core_count);
   }
}

std::pair<bool, SInt32> NetworkModelEMeshHopByHop::computeCoreCountConstraints(SInt32 core_count)
{
...
NetworkModelEMeshHopByHop::computeMemoryControllerPositions(SInt32 num_memory_controllers, SInt32 core_count)
{
   ...
   // core_id_list_along_perimeter : list of cores along the perimeter of the chip in clockwise order starting from
   // (0,0)
   ...
   LOG_ASSERT_ERROR(core_id_list_along_perimeter.size() >= (UInt32)num_memory_controllers,
                    "num cores along perimeter(%u), num memory controllers(%i)", core_id_list_along_perimeter.size(),
