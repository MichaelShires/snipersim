...
class NetworkModelEMeshHopByHop : public NetworkModel
{
 ...
 private:
   ...
   void addHop(OutputDirection direction, core_id_t final_dest, core_id_t next_dest, SubsecondTime pkt_time,
               UInt32 pkt_length, std::vector<Hop> &nextHops, core_id_t requester,
               subsecond_time_t *queue_delay_stats = NULL);
   SubsecondTime computeLatency(OutputDirection direction, SubsecondTime pkt_time, UInt32 pkt_length,
 ...
 protected:
   bool m_fake_node; //< True for nodes that are not the master of their concentrated node, these do not count in the
                     // topology
   core_id_t m_core_id;
