...
NetworkModelBusGlobal::NetworkModelBusGlobal(String name)
    : _bandwidth(8 * Sim()->getCfg()->getFloat("network/bus/bandwidth")) /* = 8 * GB/s / Gcycles/s = bits / cycle, round
                                                                            down (implicit: float to int conversion) */
      ,
      _num_packets(0), _num_packets_delayed(0), _num_bytes(0), _time_used(SubsecondTime::Zero()),
      _total_delay(SubsecondTime::Zero())
{
   String model_type = Sim()->getCfg()->getString("network/bus/queue_model/type");
   // Emulate the original code, with 10 cycles of latency for the history_list, and 0 outstanding transactions for the
   // contention model
   SubsecondTime proc_period =
       ComponentPeriod::fromFreqHz(Sim()->getCfg()->getFloatArray("perf_model/core/frequency", 0) * 1000000000);
...
SubsecondTime NetworkModelBusGlobal::useBus(SubsecondTime t_start, UInt32 length, subsecond_time_t *queue_delay_stats)
{
...
NetworkModelBus::NetworkModelBus(Network *net, EStaticNetwork net_type)
    : NetworkModel(net, net_type), _enabled(false),
...
void NetworkModelBus::routePacket(const NetPacket &pkt, std::vector<Hop> &nextHops)
{
