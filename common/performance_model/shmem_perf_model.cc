...
ShmemPerfModel::ShmemPerfModel()
    : m_enabled(false), m_num_memory_accesses(0), m_total_memory_access_latency(SubsecondTime::Zero())
{
...
void ShmemPerfModel::updateElapsedTime(SubsecondTime time, Thread_t thread_num)
{
...
void ShmemPerfModel::incrElapsedTime(SubsecondTime time, Thread_t thread_num)
{
   ...
   LOG_ASSERT_ERROR(t_elapsed_time >= i_elapsed_time, "t_elapsed_time(%s) < i_elapsed_time(%s)",
                    itostr(t_elapsed_time).c_str(), itostr(i_elapsed_time).c_str());
