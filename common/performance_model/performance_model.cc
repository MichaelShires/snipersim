...
PerformanceModel::PerformanceModel(Core *core)
    : m_core(core), m_dynins_alloc(DynamicInstruction::createAllocator()), m_enabled(false), m_fastforward(false),
      m_fastforward_model(new FastforwardPerformanceModel(core, this)), m_detailed_sync(true), m_hold(false),
      m_instruction_count(0), m_elapsed_time(Sim()->getDvfsManager()->getCoreDomain(core->getId())),
...
#else
      ,
      m_instruction_queue(1024) // Need a bit more space for when the dyninsninfo items aren't coming in yet, or for a
                                // boatload of TLBMissInstructions
#endif
      ,
...
void PerformanceModel::queuePseudoInstruction(PseudoInstruction *i)
{
   ...
   if (!m_enabled) {
      // queueInstruction and pushDynamicInstructionInfo are not being called in fast-forward by using instrumentation
      // modes For queueDynamicInstruction, which are used all over the place, ignore them manually
      delete i;
      return;
   }
