...
NNBranchPredictor::NNBranchPredictor(String name, core_id_t core_id, size_t batch_length, double learning_rate)
    : BranchPredictor(name, core_id), batch_length(batch_length), optimizer(model.parameters(), learning_rate),
      alternate_predictor(std::make_unique<PentiumMBranchPredictor>(name + "_alternate", core_id))
{
}
...
static ComponentRegistrar<BranchPredictor, String, core_id_t> 
   nn_registrar("nn", [](String name, core_id_t core_id) -> BranchPredictor* {
      UInt32 batch_length = Sim()->getCfg()->getIntArray("perf_model/branch_predictor/batch_length", core_id);
      double learning_rate = Sim()->getCfg()->getFloatArray("perf_model/branch_predictor/learning_rate", core_id);
      return new NNBranchPredictor(name, core_id, batch_length, learning_rate);
   });
