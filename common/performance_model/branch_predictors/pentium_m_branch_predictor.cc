...
PentiumMBranchPredictor::PentiumMBranchPredictor(String name, core_id_t core_id)
    : BranchPredictor(name, core_id), m_pir(0), m_last_gp_hit(false), m_last_lpb_hit(false)
{
}
...
void PentiumMBranchPredictor::update(bool predicted, bool actual, bool indirect, IntPtr ip, IntPtr target)
{
   ...
   bool lpb_or_bm_hit =
       m_last_lpb_hit ||
       m_last_bm_pred == actual; // Global should only allocate when no loop predictor hit and bimodal was wrong
   if (m_last_gp_hit) {
      if (predicted != actual && lpb_or_bm_hit) // Evict from global when mispredict and loop or bimodal hit
         ...
         m_global_predictor.update(predicted, actual, indirect, ip, target, m_pir);
            ...
            (m_last_gp_hit || !lpb_or_bm_hit)) // Update on mispredict, but don't allocate when loop or bimodal hit
      m_global_predictor.update(predicted, actual, indirect, ip, target, m_pir);
...
void PentiumMBranchPredictor::update_pir(bool actual, IntPtr ip, IntPtr target,
...
static ComponentRegistrar<BranchPredictor, String, core_id_t> 
   pentium_m_registrar("pentium_m", [](String name, core_id_t core_id) -> BranchPredictor* {
      return new PentiumMBranchPredictor(name, core_id);
   });
