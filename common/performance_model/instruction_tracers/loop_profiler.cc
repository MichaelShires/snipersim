...
LoopProfiler::LoopProfiler(const Core *core) : m_core(core), m_total_instructions(0), m_eip_last(0)
{
}
...
LoopProfiler::~LoopProfiler()
{
   ...
   for (auto it = loops.begin(); it != loops.end(); ++it) {
      printf("%5" PRId64 "x %12" PRIxPTR " .. %12" PRIx64 ": %9" PRId64 " (%5.1f%%)\n", (*it)->count, (*it)->eip,
             (*it)->eip + (*it)->size, (*it)->weight(), 100. * (*it)->weight() / m_total_instructions);
      if (--count == 0)
         break;
   }
}
...
void LoopProfiler::traceInstruction(const DynamicMicroOp *uop, uop_times_t *times)
{
