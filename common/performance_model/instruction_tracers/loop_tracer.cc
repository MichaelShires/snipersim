...
LoopTracer::LoopTracer(const Core *core)
    : m_core(core), m_address_base(strtol(
                        Sim()->getCfg()->getStringArray("loop_tracer/base_address", core->getId()).c_str(), NULL, 16)),
      m_iter_start(Sim()->getCfg()->getIntArray("loop_tracer/iter_start", core->getId())),
      m_iter_count(Sim()->getCfg()->getIntArray("loop_tracer/iter_count", core->getId())), m_active(false),
      m_iter_current(-1), m_iter_instr(0), m_instr_uop(0), m_cycle_min(UINT64_MAX), m_cycle_max(0), m_disas_max(0),
      m_instructions()
{
...
LoopTracer::~LoopTracer()
{
   ...
   for (Instructions::iterator it = m_instructions.begin(); it != m_instructions.end(); ++it) {
      printf("[%8" PRIxPTR "] %-*s (%u)    ", it->second.instruction->getAddress(), int(m_disas_max),
             it->second.instruction->getDisassembly().c_str(), it->second.uop_num);
...
void LoopTracer::traceInstruction(const DynamicMicroOp *uop, uop_times_t *times)
{
