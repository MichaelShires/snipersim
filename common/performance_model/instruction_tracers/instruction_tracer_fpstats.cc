...
InstructionTracerFPStats::InstructionTracerFPStats(const Core *core) : m_core(core)
{
   ...
   for (std::unordered_map<int, uint64_t>::iterator iclass = m_iclasses.begin(); iclass != m_iclasses.end(); ++iclass) {
      registerStatsMetric("instruction_tracer", core->getId(), fp_iclasses[iclass2index[iclass->first]],
                          &(iclass->second));
   }
}
