...
Instruction::Instruction(InstructionType type) : m_type(type), m_uops(NULL), m_addr(0)
{
}
...
PseudoInstruction::PseudoInstruction(SubsecondTime cost, InstructionType type) : Instruction(type), m_cost(cost)
{
}
...
{
   LOG_ASSERT_ERROR(
       false, "SyncInstruction::getCost() called, this instruction should not have made it into handleInstruction");
...
{
   LOG_ASSERT_ERROR(
       false, "SpawnInstruction::getCost() called, this instruction should not have made it into handleInstruction");
   return SubsecondTime::Zero();
}
...
BranchInstruction::BranchInstruction(OperandList &l) : Instruction(INST_BRANCH, l)
{
}
