...
enum InstructionType
{
   ...
   INST_SPAWN,
   INST_TLB_MISS,
   INST_MEM_ACCESS, // Not a regular memory access.  These are added latencies and do not correspond to a particular
...
__attribute__((unused)) static const char *INSTRUCTION_NAMES[] = {
    ...
    "branch",  "dynamic_misc", "recv", "sync", "spawn", "tlb_miss", "mem_access", "delay", "unknown"};
...
{
 public:
   GenericInstruction(OperandList &operands) : Instruction(INST_GENERIC, operands)
   {
   }
};
...
{
 public:
   ArithInstruction(InstructionType type, OperandList &operands) : Instruction(type, operands)
   {
   }
};
...
{
 public:
   UnknownInstruction(SubsecondTime cost) : PseudoInstruction(cost, INST_UNKNOWN)
   {
   }
};
