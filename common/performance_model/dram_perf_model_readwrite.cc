...
DramPerfModelReadWrite::DramPerfModelReadWrite(core_id_t core_id, UInt32 cache_block_size)
    : DramPerfModel(core_id, cache_block_size), m_queue_model_read(NULL), m_queue_model_write(NULL),
      m_dram_bandwidth(8 *
                       Sim()->getCfg()->getFloat("perf_model/dram/per_controller_bandwidth")), // Convert bytes to bits
      m_shared_readwrite(Sim()->getCfg()->getBool("perf_model/dram/readwrite/shared")),
      m_total_read_queueing_delay(SubsecondTime::Zero()), m_total_write_queueing_delay(SubsecondTime::Zero()),
      m_total_access_latency(SubsecondTime::Zero())
{
   m_dram_access_cost =
       SubsecondTime::FS() * static_cast<uint64_t>(TimeConverter<float>::NStoFS(Sim()->getCfg()->getFloat(
                                 "perf_model/dram/latency"))); // Operate in fs for higher precision before converting
                                                               // to uint64_t/SubsecondTime

   if (Sim()->getCfg()->getBool("perf_model/dram/queue_model/enabled")) {
      m_queue_model_read =
          QueueModel::create("dram-queue-read", core_id, Sim()->getCfg()->getString("perf_model/dram/queue_model/type"),
                             m_dram_bandwidth.getRoundedLatency(8 * cache_block_size)); // bytes to bits
      m_queue_model_write = QueueModel::create(
          "dram-queue-write", core_id, Sim()->getCfg()->getString("perf_model/dram/queue_model/type"),
          m_dram_bandwidth.getRoundedLatency(8 * cache_block_size)); // bytes to bits
   }
...
SubsecondTime DramPerfModelReadWrite::getAccessLatency(SubsecondTime pkt_time, UInt64 pkt_size, core_id_t requester,
