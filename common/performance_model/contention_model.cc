...
ContentionModel::ContentionModel()
    : m_num_outstanding(1), m_time(m_num_outstanding, std::make_pair(SubsecondTime::Zero(), 0)),
      m_t_last(SubsecondTime::Zero()), m_proc_period(NULL), m_n_requests(0), m_n_barriers(0), m_n_outoforder(0),
      m_n_simultaneous(0), m_n_hasfreefail(0), m_total_delay(SubsecondTime::Zero()),
...
ContentionModel::ContentionModel(String name, core_id_t core_id, UInt32 num_outstanding)
    : m_num_outstanding(num_outstanding), m_time(m_num_outstanding, std::make_pair(SubsecondTime::Zero(), 0)),
      m_t_last(SubsecondTime::Zero()), m_proc_period(Sim()->getDvfsManager()->getCoreDomain(core_id)), m_n_requests(0),
      m_n_barriers(0), m_n_outoforder(0), m_n_simultaneous(0), m_n_hasfreefail(0), m_total_delay(SubsecondTime::Zero()),
      m_total_barrier_delay(SubsecondTime::Zero())
{
...
uint64_t ContentionModel::getBarrierCompletionTime(uint64_t t_start, uint64_t t_delay, UInt64 tag)
{
   SubsecondTimeCycleConverter conv(m_proc_period);
   SubsecondTime result =
       getBarrierCompletionTime(conv.cyclesToSubsecondTime(t_start), conv.cyclesToSubsecondTime(t_delay), tag);
   return conv.subsecondTimeToCycles(result);
}

SubsecondTime ContentionModel::getBarrierCompletionTime(SubsecondTime t_start, SubsecondTime t_delay, UInt64 tag)
{
   if (m_num_outstanding == 0)
...
uint64_t ContentionModel::getCompletionTime(uint64_t t_start, uint64_t t_delay, UInt64 tag)
{
   SubsecondTimeCycleConverter conv(m_proc_period);
   SubsecondTime result =
       getCompletionTime(conv.cyclesToSubsecondTime(t_start), conv.cyclesToSubsecondTime(t_delay), tag);
   return conv.subsecondTimeToCycles(result);
}

/* Model utilization. In: start time and utilization time. Out: completion time */
SubsecondTime ContentionModel::getCompletionTime(SubsecondTime t_start, SubsecondTime t_delay, UInt64 tag)
{
   if (m_num_outstanding == 0)
      return t_start + t_delay;
