...
QueueModelHistoryList::QueueModelHistoryList(String name, UInt32 id, SubsecondTime min_processing_time)
    : m_min_processing_time(min_processing_time), m_utilized_time(SubsecondTime::Zero()),
      m_total_queue_delay(SubsecondTime::Zero()), m_total_requests(0), m_total_requests_using_analytical_model(0)
{
   ...
   m_average_delay =
       MovingAverage<SubsecondTime>::createAvgType(MovingAverage<SubsecondTime>::ARITHMETIC_MEAN, max_list_size);
   SubsecondTime max_simulation_time = SubsecondTime::FS() << 63;
   m_free_interval_list.push_back(
       std::pair<const SubsecondTime, SubsecondTime>(SubsecondTime::Zero(), max_simulation_time));
...
SubsecondTime QueueModelHistoryList::computeQueueDelay(SubsecondTime pkt_time, SubsecondTime processing_time,
                                                       core_id_t requester)
{
   LOG_ASSERT_ERROR(m_free_interval_list.size() >= 1, "Free Interval list size < 1");
...
void QueueModelHistoryList::updateQueueUtilization(SubsecondTime processing_time)
{
...
SubsecondTime QueueModelHistoryList::computeUsingAnalyticalModel(SubsecondTime pkt_time, SubsecondTime processing_time)
{
   ...
   //     For rho near or over 1, which does happen due to other approximations (skew between cores), this yields
   //     nonsense. Yet, everyone knows that in a system with feedback, the arrival rate is no longer an independent,
   //     exponential process (i.e., more delay causes upstream buffer blockages which throttle the actual request rate,
   //     avoiding rho from ever reaching 1)
   // Current best guess: return average of delays computed using history list model
   return m_average_delay->compute();
}

SubsecondTime QueueModelHistoryList::computeUsingHistoryList(SubsecondTime pkt_time, SubsecondTime processing_time)
{
   LOG_ASSERT_ERROR(m_free_interval_list.size() <= m_max_free_interval_list_size, "Free Interval list size(%u) > %u",
   ...
   for (curr_it = m_free_interval_list.begin(); curr_it != m_free_interval_list.end(); curr_it++) {
      ...
      if ((pkt_time >= interval.first) && ((pkt_time + processing_time) <= interval.second)) {
         ...
         if ((interval.second - (pkt_time + processing_time)) >= m_min_processing_time) {
            m_free_interval_list.insert(
                curr_it, std::pair<SubsecondTime, SubsecondTime>(pkt_time + processing_time, interval.second));
         }
         break;
      }
      ...
      //      so this won't be accurate.)
         ...
         if ((interval.second - (interval.first + processing_time)) >= m_min_processing_time) {
            m_free_interval_list.insert(
                curr_it, std::pair<SubsecondTime, SubsecondTime>(interval.first + processing_time, interval.second));
         }
         break;
      }
   }

   LOG_ASSERT_ERROR(queue_delay != SubsecondTime::MaxTime(), "queue delay(%s), free interval not found",
   ...
   LOG_PRINT("HistoryList: pkt_time(%s), processing_time(%s), queue_delay(%s)", itostr(pkt_time).c_str(),
             itostr(processing_time).c_str(), itostr(queue_delay).c_str());
