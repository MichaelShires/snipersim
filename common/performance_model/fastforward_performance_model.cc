...
FastforwardPerformanceModel::FastforwardPerformanceModel(Core *core, PerformanceModel *perf)
    : m_core(core), m_perf(perf),
      ...
      m_include_branch_mispredict(
          Sim()->getCfg()->getBool("perf_model/fast_forward/oneipc/include_branch_misprediction")),
      m_branch_misprediction_penalty(
          core->getDvfsDomain(),
          Sim()->getCfg()->getIntArray("perf_model/branch_predictor/mispredict_penalty", core->getId())),
      m_cpi(SubsecondTime::Zero()), m_fastforwarded_time(SubsecondTime::Zero())
{
...
void FastforwardPerformanceModel::incrementElapsedTime(SubsecondTime latency, SubsecondTime &cpiComponent)
{
...
void FastforwardPerformanceModel::countInstructions(IntPtr address, UInt32 count)
{
   incrementElapsedTime(count * m_cpi, m_cpiBase);
}

void FastforwardPerformanceModel::handleMemoryLatency(SubsecondTime latency, HitWhere::where_t hit_where)
{
...
void FastforwardPerformanceModel::queuePseudoInstruction(PseudoInstruction *instr)
{
