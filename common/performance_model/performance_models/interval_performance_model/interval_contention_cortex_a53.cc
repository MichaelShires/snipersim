    ...
    : m_core_model(core_model)
{
   for (unsigned int i = 0; i < DynamicMicroOpCortexA53::UOP_PORT_SIZE; ++i) {
      m_cpContrByPort[i] = 0;
      String name =
          String("cpContr_") + DynamicMicroOpCortexA53::PortTypeString((DynamicMicroOpCortexA53::uop_port_t)i);
...
uint64_t IntervalContentionCortexA53::getEffectiveCriticalPathLength(uint64_t critical_path_length, bool update_reason)
{
   ...
   /*   for (unsigned int i = 0 ; i < DynamicMicroOpCortexA53::UOP_PORT_SIZE ; i++)
      ...
      uint64_t port05_use = m_count_byport[DynamicMicroOpCortexA53::UOP_PORT0] +
      m_count_byport[DynamicMicroOpCortexA53::UOP_PORT5]
      ...
      uint64_t port015_use = m_count_byport[DynamicMicroOpCortexA53::UOP_PORT0] +
      m_count_byport[DynamicMicroOpCortexA53::UOP_PORT1]
                           + m_count_byport[DynamicMicroOpCortexA53::UOP_PORT5] +
      m_count_byport[DynamicMicroOpCortexA53::UOP_PORT015]; if (port015_use > (3*effective_cp_length))
      {
      ...
      {
         LOG_ASSERT_ERROR(reason != DynamicMicroOpCortexA53::UOP_PORT_SIZE, "There should be a reason for the cp
      extention, but there isn't"); m_cpContrByPort[reason] += 1000000 * (effective_cp_length - critical_path_length) /
      effective_cp_length;
      }*/
