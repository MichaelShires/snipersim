...
class Windows
{
 public:
   struct WindowEntry
   {
      ...
      /** The latency of the microInstruction can be overlapped by a long latency load. The flag states what is
       * overlapped: a icache miss, a branch mispredict or a dcache miss. */
      ...
      /** Used during the block window algorithm, shouldn't be here -> has to be int[doubleWindowSize] in IntervalTimer
       * or Windows. */
      uint32_t dependent;
      ...
      {
         return this->dependent == DATA_DEP;
      }
      bool isIndependent() const
      {
         return this->dependent == INDEP_MISS;
      }
      void setDataDependent()
      {
         this->dependent = DATA_DEP;
      }
      void setIndependentMiss()
      {
         this->dependent = INDEP_MISS;
      }
   };
 ...
 private:
   ...
   uint32_t *const m_exec_time_map; // Used to store the execution time of the producers when calculating the branch
                                    // resolution time.
