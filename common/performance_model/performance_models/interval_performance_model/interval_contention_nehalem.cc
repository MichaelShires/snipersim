...
uint64_t IntervalContentionNehalem::getEffectiveCriticalPathLength(uint64_t critical_path_length, bool update_reason)
{
   ...
   for (unsigned int i = 0; i < DynamicMicroOpNehalem::UOP_PORT_SIZE; i++) {
      // Skip shared ports
      if (i != DynamicMicroOpNehalem::UOP_PORT015 && i != DynamicMicroOpNehalem::UOP_PORT05 &&
   ...
   uint64_t port015_use =
       m_count_byport[DynamicMicroOpNehalem::UOP_PORT0] + m_count_byport[DynamicMicroOpNehalem::UOP_PORT1] +
   ...
   if (update_reason && effective_cp_length > critical_path_length) {
      LOG_ASSERT_ERROR(reason != DynamicMicroOpNehalem::UOP_PORT_SIZE,
