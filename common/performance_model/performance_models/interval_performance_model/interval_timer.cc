...
IntervalTimer::IntervalTimer(Core *core, PerformanceModel *_perf, const CoreModel *core_model,
                             int misprediction_penalty, int dispatch_width, int window_size,
                             bool do_functional_unit_contention)
    : m_core(core), m_core_model(core_model), m_dispatch_width(dispatch_width),
      m_branch_misprediction_penalty(misprediction_penalty), m_remaining_dispatch_bandwidth(0),
      m_max_store_completion_time(0), m_max_load_completion_time(0),
      ...
      m_windows(new Windows(window_size, do_functional_unit_contention, core, core_model)), m_perf_model(_perf),
      m_frequency_domain(core->getDvfsDomain())
{

   // Granularity of memory dependencies, in bytes
   UInt64 mem_gran =
       Sim()->getCfg()->getIntArray("perf_model/core/interval_timer/memory_dependency_granularity", core->getId());
   ...
   UInt64 lll_gran =
       Sim()->getCfg()->getIntArray("perf_model/core/interval_timer/lll_dependency_granularity", core->getId());
   ...
   registerStatsMetric("interval_timer", core->getId(), "numTotalLongLatencyLoadLatency",
   ...
   registerStatsMetric("interval_timer", core->getId(), "totalHiddenLongerDCacheLatency",
                       &m_totalHiddenLongerDCacheLatency);
   registerStatsMetric("interval_timer", core->getId(), "numHiddenLongerDCacheLatency",
   ...
   registerStatsMetric("interval_timer", core->getId(), "outstandingLongLatencyCycles",
   ...
   for (std::vector<DynamicMicroOp *>::const_iterator i = insts.begin(); i != insts.end(); ++i) {
      ...
      if (m_uops_total > 10000 && m_uops_x87 > m_uops_total / 20) {
         LOG_PRINT_WARNING_ONCE("Significant fraction of x87 instructions encountered, accuracy will be low. Compile "
                                "without -mno-sse2 -mno-sse3 to avoid these instructions.");
      }
...
boost::tuple<uint64_t, uint64_t> IntervalTimer::dispatchWindow()
{
   ...
   SubsecondTime micro_op_period =
       m_frequency_domain->getPeriod(); // Approximate the current cycle count if we don't find a MicroOp

   // Instruction dispatch
   // micro_ops_executed must be < dispatch_rate.  micro_ops_executed <= dispatch_rate will cause significant speedups
   // (see calculateCurrentDispatchRate())
   while ((!m_windows->wIsEmpty()) && (instructions_executed < m_dispatch_width) &&
          (micro_ops_executed < dispatch_rate) && (continue_dispatching == STOP_DISPATCH_NO_REASON))
   {
...
uint32_t IntervalTimer::calculateCurrentDispatchRate()
{
   ...
   if (critical_path_length > 0) {
      FixedPoint ipc = FixedPoint(m_windows->getOldWindowLength()) /
                           m_windows->getEffectiveCriticalPathLength(critical_path_length, false) +
...
void IntervalTimer::issueMemOp(Windows::WindowEntry &micro_op)
{
   ...
   // - not at all if all memory operations are issued at fetch (perf_model/core/interval_timer/issue_memops_at_dispatch
   // == false)
   ...
   {
      MemoryResult res = m_core->accessMemory(Core::NONE, micro_op.getMicroOp()->isLoad() ? Core::READ : Core::WRITE,
      ...
      micro_op.getDynMicroOp()->setExecLatency(micro_op.getDynMicroOp()->getExecLatency() +
...
uint64_t IntervalTimer::dispatchInstruction(Windows::WindowEntry &micro_op, StopDispatchReason &continue_dispatching)
{
   ...
   bool icache_miss = (micro_op.getDynMicroOp()->getICacheHitWhere() != HitWhere::L1I) &
   ...
   if (micro_op.getMicroOp()->isSerializing() || micro_op.getMicroOp()->isInterrupt()) {
      uint64_t flushLatency =
          std::max(m_windows->getCriticalPathLength(), m_windows->getMinimalFlushLatency(m_dispatch_width));
      ...
      uint64_t cycle_to_wait_until =
          std::max(max_producer_exec_time, std::max(m_max_store_completion_time, m_max_load_completion_time));
      ...
      if (micro_op.getMicroOp()->isLoad()) {
         if (micro_op.hasOverlapFlag(Windows::WindowEntry::DCACHE_OVERLAP)) {
            // do nothing for overlapped loads, as they don't add to the critical path and happen in parallel to other
            // loads (MLP > 1)
         }
            ...
            uint64_t long_latency_load_latency =
                ...
                contention_extra_latency; // dcache_latency is already taken into account in contention_extra_latency
            ...
            fprintf(m_insn_log, ") latency=%ld (%ld+%ld(%ld))\n", latency, reswin_extra_latency,
                    contention_extra_latency, dcache_latency);
#endif
         ...
         uint64_t data_ready_cycle =
             sched_cycle + bypass_latency + 1;   // This store result will be ready to use one cycle later
         ...
         uint64_t exec_time_cycle =
             sched_cycle + store_latency; // For future instructions that depend on this instruction's result
         m_max_store_completion_time = std::max(m_max_store_completion_time, exec_time_cycle);

         // FIXME: Don't update micro_op.setExecTime (else it will eventually affect the critical path head, once this
         // instruction goes through Windows::dispatchInstruction). If we need this time, figure out another place to
         // put it.
...
{
   int64_t oldestStartTime = m_windows->getOldestInstruction().getExecTime() -
