 ...
 * this->sequenceNumber - typeOffset (= sequenceNumber of first microOperation with the current type) -
...
struct MicroOp
{
   ...
   /** The intraInstructionDependencies variable gives the number of preceding microOp on which this micrOp depends.
    * This number is counted from the first microOp with that type. */
   ...
   /** This array contains the registers read by this MicroOperation, the integer is an id given by libdisasm64. Only
    * valid for UOP_LOAD and UOP_EXECUTE. */
   ...
   /** This array contains the registers written by this MicroOperation, the integer is an id given by libdisasm64. Only
    * valid for UOP_EXECUTE. */
   ...
   void makeLoad(uint32_t offset, dl::Decoder::decoder_opcode instructionOpcode, const String &instructionOpcodeName,
                 uint16_t mem_size);
   void makeExecute(uint32_t offset, uint32_t num_loads, dl::Decoder::decoder_opcode instructionOpcode,
                    const String &instructionOpcodeName, bool isBranch);
   void makeStore(uint32_t offset, uint32_t num_execute, dl::Decoder::decoder_opcode instructionOpcode,
   ...
   {
      this->m_membar = membar;
   }
   bool isCacheFlush() const
   {
      return Sim()->getDecoder()->is_cache_flush_opcode(this->instructionOpcode);
   }
   bool isDiv() const
   {
      return Sim()->getDecoder()->is_div_opcode(this->instructionOpcode);
   }
   bool isPause() const
   {
      return Sim()->getDecoder()->is_pause_opcode(this->instructionOpcode);
   }

   bool isLoad() const
   {
      return this->uop_type == UOP_LOAD;
   }
   bool isStore() const
   {
      return this->uop_type == UOP_STORE;
   }
   ...
   {
      return uop_type == UOP_EXECUTE;
   }

   bool isWriteback() const
   {
      return decodedInstruction->is_writeback();
   }
...
#ifdef ENABLE_MICROOP_STRINGS
   const String &getInstructionOpcodeName()
   {
      return instructionOpcodeName;
   }
#endif
};
