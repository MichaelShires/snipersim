...
class DynamicMicroOp
{
 ...
 public:
   ...
   template <typename T>
   static T *alloc(Allocator *alloc, const MicroOp *uop, const CoreModel *core_model, ComponentPeriod period)
   {
   ...
   template <typename T> const T *getCoreSpecificInfo() const
   {
      ...
      LOG_ASSERT_ERROR(ptr != NULL, "DynamicMicroOp of the wrong type: expected %s, got %s", /*T::getType()*/ "???",
                       this->getType());
      return ptr;
   }
   ...
   {
      return microOpTypeOffset;
   }
   void setMicroOpTypeOffset(uint32_t offset)
   {
      microOpTypeOffset = offset;
   }
   ...
   {
      this->branchTargetAddress = address;
   }
   ...
   {
      return this->execLatency;
   }
   void setExecLatency(uint32_t latency)
   {
      this->execLatency = latency;
   }
   ...
   {
      LOG_ASSERT_ERROR(m_period != SubsecondTime::Zero(), "MicroOp Period is == SubsecondTime::Zero()");
      return m_period;
   }
