...
RegisterDependencies::RegisterDependencies() : producers(Sim()->getDecoder()->last_reg())
{
   clear();
}
...
void RegisterDependencies::setDependencies(DynamicMicroOp &microOp, uint64_t lowestValidSequenceNumber)
{
   ...
   for (uint32_t i = 0; i < microOp.getMicroOp()->getSourceRegistersLength(); i++) {
      ...
      LOG_ASSERT_ERROR(sourceRegister < Sim()->getDecoder()->last_reg(), "Source register src[%u]=%u is invalid", i,
                       sourceRegister);
   ...
   for (uint32_t i = 0; i < microOp.getMicroOp()->getDestinationRegistersLength(); i++) {
      ...
      LOG_ASSERT_ERROR(destinationRegister < Sim()->getDecoder()->last_reg(),
                       "Destination register dst[%u] = %u is invalid", i, destinationRegister);
