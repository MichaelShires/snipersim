...
unsigned int CoreModelNehalem::getInstructionLatency(const MicroOp *uop) const
{
   ...
   LOG_ASSERT_ERROR(instruction_type >= 0 && instruction_type < XED_ICLASS_LAST, "Invalid instruction type %d",
                    instruction_type);
...
unsigned int CoreModelNehalem::getBypassLatency(const DynamicMicroOp *uop) const
{
   ...
   LOG_ASSERT_ERROR(bypass_type >= 0 && bypass_type < DynamicMicroOpNehalem::UOP_BYPASS_SIZE, "Invalid bypass type %d",
                    bypass_type);
...
DynamicMicroOp *CoreModelNehalem::createDynamicMicroOp(Allocator *alloc, const MicroOp *uop,
