...
DynamicMicroOpNehalem::DynamicMicroOpNehalem(const MicroOp *uop, const CoreModel *core_model, ComponentPeriod period)
    : DynamicMicroOp(uop, core_model, period), uop_port(DynamicMicroOpNehalem::getPort(uop)),
      uop_alu(DynamicMicroOpNehalem::getAlu(uop)), uop_bypass(DynamicMicroOpNehalem::getBypassType(uop))
{
}
