...
unsigned int CoreModelBoomV1::getInstructionLatency(const MicroOp *uop) const
{
   ...
   LOG_ASSERT_ERROR(instruction_type > 0 && instruction_type < rv_op_last, "Invalid instruction type %d",
                    instruction_type);
...
unsigned int CoreModelBoomV1::getBypassLatency(const DynamicMicroOp *uop) const
{
   ...
   LOG_ASSERT_ERROR(bypass_type >= 0 && bypass_type < DynamicMicroOpBoomV1::UOP_BYPASS_SIZE, "Invalid bypass type %d",
                    bypass_type);
...
DynamicMicroOp *CoreModelBoomV1::createDynamicMicroOp(Allocator *alloc, const MicroOp *uop,
