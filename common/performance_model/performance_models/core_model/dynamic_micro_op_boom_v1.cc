...
{
   if (instrlist[uop->getInstructionOpcode()].has_fpu || instrlist[uop->getInstructionOpcode()].has_fdiv ||
       instrlist[uop->getInstructionOpcode()].has_mul)
   {
...
DynamicMicroOpBoomV1::DynamicMicroOpBoomV1(const MicroOp *uop, const CoreModel *core_model, ComponentPeriod period)
    : DynamicMicroOp(uop, core_model, period), uop_port(DynamicMicroOpBoomV1::getPort(uop)),
      uop_alu(DynamicMicroOpBoomV1::getAlu(uop)), uop_bypass(DynamicMicroOpBoomV1::getBypassType(uop))
{
}
