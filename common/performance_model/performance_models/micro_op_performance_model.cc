...
MicroOpPerformanceModel::MicroOpPerformanceModel(Core *core, bool issue_memops)
    : PerformanceModel(core), m_core_model(CoreModel::getCoreModel(
                                  Sim()->getCfg()->getStringArray("perf_model/core/core_model", core->getId()))),
      m_allocator(m_core_model->createDMOAllocator()), m_issue_memops(issue_memops), m_dyninsn_count(0),
      m_dyninsn_cost(0), m_dyninsn_zero_count(0)
{
   ...
   for (i = first_squashed; i < size; i++) {
      ...
      if (!uop->isSquashed()) {
         ...
         if (intraDeps != 0) {
            ...
            LOG_ASSERT_ERROR(iBase >= intraDeps, "intraInstructionDependancies (%d) should be <= (%d)", intraDeps,
                             iBase);
...
void MicroOpPerformanceModel::handleInstruction(DynamicInstruction *dynins)
{
   ...
   if (dynins->instruction->getMicroOps()) {
      for (std::vector<const MicroOp *>::const_iterator it = dynins->instruction->getMicroOps()->begin();
         ...
         if (o.m_direction == Operand::READ) {
            ...
            if (load_base_index != SIZE_MAX) {
               ...
               LOG_ASSERT_ERROR(load_index < m_current_uops.size(),
                                "Expected load_index(%x) to be less than uops.size()(%d).", load_index,
                                m_current_uops.size());
               LOG_ASSERT_ERROR(m_current_uops[load_index]->getMicroOp()->isLoad(), "Expected uop %d to be a load.",
                                load_index);

               if (std::find(m_cache_lines_read.begin(), m_cache_lines_read.end(), cache_line) !=
                   m_cache_lines_read.end())
               {
            ...
            if (store_base_index != SIZE_MAX) {
               ...
               LOG_ASSERT_ERROR(store_index < m_current_uops.size(),
                                "Expected store_index(%d) to be less than uops.size()(%d).", store_index,
                                m_current_uops.size());
               ...
               if (std::find(m_cache_lines_written.begin(), m_cache_lines_written.end(), cache_line) !=
   ...
   if (dynins->instruction->getType() == INST_BRANCH) {
      ...
      if (insn_cost > 17) {
         ...
         if (exec_base_index != SIZE_MAX) {
            ...
            fprintf(m_insn_log, "-%s:%s", dynins->instruction->getDisassembly().c_str(),
   ...
   bool latency_out_of_band = false; // If new_latency contains any values not returned from IntervalTimer/RobTimer,
                                     // we'll need to call synchronize()
   if (m_current_uops.size() > 0) {
      ...
      fprintf(m_dyninsn_log, "[%llu] %s: cost = %llu\n", (long long unsigned int)m_elapsed_time,
              dynins->instruction->getTypeName().c_str(), (long long unsigned int)insn_cost);
#endif
   }
      ...
      if ((cutoff > 0) && (insn_cost > cutoff * insn_period.getPeriod())) {
         ...
         // FIXME For possible next steps, be sure to pass real LLL value in when we want to compare LLL event lengths
         // for higher accuracy
         force_lll = true;
      }
      else {
