...
RobSmtTimer *RobSmtPerformanceModel::getRobTimer(Core *core, RobSmtPerformanceModel *perf, const CoreModel *core_model)
{
   ...
   if (s_rob_timers.count(core_id_master) == 0) {
      s_rob_timers[core_id_master] =
          new RobSmtTimer(smt_threads, core, perf, core_model,
...
RobSmtPerformanceModel::RobSmtPerformanceModel(Core *core)
    : MicroOpPerformanceModel(
          core, !Sim()->getCfg()->getBoolArray("perf_model/core/rob_timer/issue_memops_at_issue", core->getId())),
      m_rob_timer(getRobTimer(core, this, m_core_model)), m_enabled(true)
{
