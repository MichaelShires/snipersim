...
class RobSmtTimer : public SmtTimer
{
 private:
   class RobEntry
   {
    ...
    public:
      ...
      {
         LOG_ASSERT_ERROR(numAddressProducers < MAX_ADDRESS_PRODUCERS,
                          "Too many address producers, increase MAX_ADDRESS_PRODUCERS(%d)", MAX_ADDRESS_PRODUCERS);
         addressProducers[numAddressProducers++] = sequenceNumber;
      }
      size_t getNumAddressProducers() const
      {
         return numAddressProducers;
      }
      ...
      SubsecondTime readyMax; // While some but not all dependencies are resolved, keep the time of the latest known
 ...
 public:
   RobSmtTimer(int num_threads, Core *core, PerformanceModel *perf, const CoreModel *core_model,
