...
RobSmtTimer::RobThread::RobThread(Core *_core, int window_size)
    : core(_core), now(SubsecondTime::Zero()), instrs(0), instrs_returned(0), rob(window_size + 255), m_num_in_rob(0),
      nextSequenceNumber(0), frontend_stalled_until(SubsecondTime::Zero()), in_icache_miss(false),
      next_event(SubsecondTime::Zero()), registerDependencies(new RegisterDependencies()),
      memoryDependencies(new MemoryDependencies()), m_cpiCurrentFrontEndStall(&m_cpiSMT)
{
}
...
RobSmtTimer::RobSmtTimer(int num_threads, Core *core, PerformanceModel *_perf, const CoreModel *core_model,
                         int misprediction_penalty, int dispatch_width, int window_size)
    : SmtTimer(num_threads), dispatchWidth(dispatch_width),
      ...
      m_rob_contention(Sim()->getCfg()->getBoolArray("perf_model/core/rob_timer/issue_contention", core->getId())
                           ...
                           : NULL),
      dispatch_thread(0), issue_thread(0), now(core->getDvfsDomain()), last_store_done(SubsecondTime::Zero()),
      ...
      m_rs_entries_used(0), will_skip(false), time_skipped(SubsecondTime::Zero()),
...
void RobSmtTimer::initializeThread(smtthread_id_t thread_num)
{
   ...
   registerStatsMetric("rob_timer", core->getId(), "outstandingLongLatencyCycles",
...
void RobSmtTimer::setDependencies(smtthread_id_t thread_id, RobEntry *entry)
{
   ...
   uint64_t lowestValidSequenceNumber =
       thread->rob.size() > 0 ? thread->rob.front().uop->getSequenceNumber() : entry->uop->getSequenceNumber();
   ...
   for (unsigned int i = 0; i < entry->uop->getDependenciesLength(); ++i) {
      ...
      if (dependencySequenceNumber < lowestValidSequenceNumber) {
         // dependency is already done (this will be an intra-instruction dependency, register/memoryDependencies won't
         // return anything < lowestValidSequenceNumber)
...
void RobSmtTimer::setStoreAddressProducers(smtthread_id_t thread_id, RobEntry *entry,
                                           uint64_t lowestValidSequenceNumber)
{
   RobThread *thread = m_rob_threads[thread_id];
...
#ifdef DEBUG_PERCYCLE
      std::cout << "** [" << int(thread_id) << "] simulate: " << entry->uop->getMicroOp()->toShortString(true)
                << std::endl;
#endif
...
boost::tuple<uint64_t, SubsecondTime> RobSmtTimer::returnLatency(smtthread_id_t thread_id)
{
   ...
   if (returnLat > SubsecondTime::Zero())
      std::cout << "** [" << int(thread_id) << "] return " << returnLat << " latency cycles" << std::endl
                << std::endl
                << std::endl;
#endif
...
#ifdef DEBUG_PERCYCLE
   std::cout << "** [" << int(thread_id) << "] jump ahead after wake by " << (time - m_rob_threads[thread_id]->now)
             << " cycles" << std::endl;
#endif
...
#ifdef DEBUG_PERCYCLE
      std::cout << "** [" << int(thread_id) << "] fast-forwarding core time to " << toCycles(now) << " cycles"
                << std::endl;
#endif
   }
}
...
SubsecondTime RobSmtTimer::doDispatch()
{
   ...
   // Or no-one issued, then thread_dispatched_from == thread_first, so we'll continue with the one following the
...
#ifdef DEBUG_PERCYCLE
   std::cout << "[" << int(thread_num) << "] ISSUE    " << entry->uop->getMicroOp()->toShortString()
...
#ifdef ASSERT_SKIP
            LOG_ASSERT_ERROR(thread->m_outstandingLongLatencyInsns >= thread->m_outstandingLongLatencyCycles,
...
SubsecondTime RobSmtTimer::doIssue()
{
   ...
   // Either someone has issued, in the following cycle we'll start with the one following the thread that did issue
   // Or no-one issued, then thread_num == thread_first, so we'll continue with the one following the previous first
   // thread
...
void RobSmtTimer::printRob(smtthread_id_t thread_num)
{
   ...
   SmtThread *smt_thread = m_threads[thread_num];
   std::cout << "** State:";
   if (smt_thread->in_wakeup)
      std::cout << " IN_WAKEUP";
   if (smt_thread->running)
      std::cout << " RUNNING";
   if (smt_thread->in_barrier)
      std::cout << " BARRIER";
   std::cout << std::endl;
#endif
   ...
   if (thread->rob.size() > thread->m_num_in_rob + 16)
      std::cout << "   -----  " << (thread->rob.size() - (thread->m_num_in_rob + 16)) << " more PREROB -----"
                << std::endl;
}
