...
FaultinjectionManager *FaultinjectionManager::create(SimulationContext *context)
{
   config::Config *cfg = context->getConfigFile();
   String s_type = cfg->getString("fault_injection/type");
   String s_injector = cfg->getString("fault_injection/injector");
...
FaultInjector *FaultinjectionManager::getFaultInjector(UInt32 core_id, MemComponent::component_t mem_component)
{
   switch (m_injector) {
...
void FaultinjectionManager::applyFault(Core *core, IntPtr read_address, UInt32 data_size, MemoryResult &memres,
...
void FaultInjector::preRead(IntPtr addr, IntPtr location, UInt32 data_size, Byte *fault, SubsecondTime time)
{
...
void FaultInjector::postWrite(IntPtr addr, IntPtr location, UInt32 data_size, Byte *fault, SubsecondTime time)
{
