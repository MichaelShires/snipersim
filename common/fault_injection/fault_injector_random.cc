...
void FaultInjectorRandom::preRead(IntPtr addr, IntPtr location, UInt32 data_size, Byte *fault, SubsecondTime time)
{
   ...
   if (m_active && (rng_next(m_rng) % 0xffff) == 0) {
      ...
      printf("Inserting bit %d flip at address %" PRIxPTR " on read access by core %d to component %s\n", bit_location,
             addr, m_core_id, MemComponentString(m_mem_component));

      fault[bit_location / 8] |= 1 << (bit_location % 8);
   }
}

void FaultInjectorRandom::postWrite(IntPtr addr, IntPtr location, UInt32 data_size, Byte *fault, SubsecondTime time)
{
