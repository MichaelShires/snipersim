...
class TraceThread : public Runnable
{
 private:
   ...
   static void __handleCacheOnlyFunc(void *arg, uint8_t icount, Sift::CacheOnlyType type, uint64_t eip,
                                     uint64_t address)
   {
   ...
   static void __handleRoutineChangeFunc(void *arg, Sift::RoutineOpType event, uint64_t eip, uint64_t esp,
                                         uint64_t callEip)
   {
      ((TraceThread *)arg)->handleRoutineChangeFunc(event, eip, esp, callEip);
   }
   static void __handleRoutineAnnounceFunc(void *arg, uint64_t eip, const char *name, const char *imgname,
                                           uint64_t offset, uint32_t line, uint32_t column, const char *filename)
   {
   ...
   void handleRoutineAnnounceFunc(uint64_t eip, const char *name, const char *imgname, uint64_t offset, uint32_t line,
                                  uint32_t column, const char *filename);

   Instruction *decode(Sift::Instruction &inst);
   void handleInstructionWarmup(Sift::Instruction &inst, Sift::Instruction &next_inst, Core *core,
                                bool do_icache_warmup, UInt64 icache_warmup_addr, UInt64 icache_warmup_size);
   void handleInstructionDetailed(Sift::Instruction &inst, Sift::Instruction &next_inst, PerformanceModel *prfmdl);
   void addDetailedMemoryInfo(DynamicInstruction *dynins, Sift::Instruction &inst, const dl::DecodedInst &decoded_inst,
                              uint32_t mem_idx, Operand::Direction op_type, bool is_pretetch, PerformanceModel *prfmdl);
   void unblock();
 ...
 public:
   ...
   TraceThread(TraceManager *manager, SimulationContext *context, app_id_t app_id, uint32_t thread_num, String tracefile, String responsefile);
   ~TraceThread();
   ...
   void handleAccessMemory(Core::lock_signal_t lock_signal, Core::mem_op_t mem_op_type, IntPtr d_addr,
