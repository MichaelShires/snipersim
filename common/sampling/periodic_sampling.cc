...
PeriodicSampling::PeriodicSampling(SamplingManager *sampling_manager)
    : SamplingAlgorithm(sampling_manager)
      ...
      m_fastforward_sync_interval(
          SubsecondTime::NS(Sim()->getCfg()->getInt("sampling/periodic/fastforward_sync_interval")))
      ...
      m_detailed_warmup_interval(
          SubsecondTime::NS(Sim()->getCfg()->getInt("sampling/periodic/detailed_warmup_interval")))
      // Whether to simulate synchronization during fast-forward (true, our method), or fast-forward using a per-core
      // CPI that contains sync (false, COTSon method)
      ,
      ...
      m_periodic_last(SubsecondTime::Zero()), m_historic_cpi_intervals(Sim()->getConfig()->getApplicationCores(), NULL),
      m_dispatch_width(Sim()->getCfg()->getInt("perf_model/core/interval_timer/dispatch_width"))
{
   LOG_ASSERT_ERROR(m_fastforward_sync_interval > SubsecondTime::Zero() &&
                        m_fastforward_sync_interval <= std::max(m_fastforward_interval, m_warmup_interval),
                    "fastforward_sync_interval must be between 0 and max(fastforward_interval, warmup_interval)");
...
void PeriodicSampling::callbackDetailed(SubsecondTime time)
{
   if (m_random_start) {
      ...
      LOG_ASSERT_ERROR(m_detailed_warmup_time_remaining == SubsecondTime::Zero(),
                       "Should not finish detailed simulation before detailed warmup is complete.")
      // printf("IPC =");
      for (unsigned int core_id = 0; core_id < Sim()->getConfig()->getApplicationCores(); ++core_id) {
         ...
         if (m_constant_ipc) {
            ...
            SubsecondTime historic_cpi =
                m_sampling_manager->getCoreHistoricCPI(core, m_detailed_sync, m_detailed_interval / 5);
      ...
      // If detailed warmup is enabled and we have simulated the requested detailed amount, reset the statistics and
      // continue in detailed
