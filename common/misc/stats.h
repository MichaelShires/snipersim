...
class StatsManager
{
 public:
   ...
   {
      EVENT_MARKER =
          1, // calling core      calling thread    magic arg0     magic arg1        str (SimMarker/SimNamedMarker)
      EVENT_THREAD_NAME,   // calling core      calling thread    0              0                 thread name
                           // (SimSetThreadName)
   ...
   void logMarker(SubsecondTime time, core_id_t core_id, thread_id_t thread_id, UInt64 value0, UInt64 value1,
                  const char *description)
   {
      logEvent(EVENT_MARKER, time, core_id, thread_id, value0, value1, description);
   }
   void logEvent(event_type_t event, SubsecondTime time, core_id_t core_id, thread_id_t thread_id, UInt64 value0,
                 UInt64 value1, const char *description);
 ...
 private:
   ...
   // Use std::string here because String (__versa_string) does not provide a hash function for STL containers with gcc
   // < 4.6
...
template <class T> void registerStatsMetric(String objectName, UInt32 index, String metricName, T *metric, StatsManager *stats_manager = NULL)
{
   if (!stats_manager) {
      if (Simulator::getSingleton() && Simulator::getSingleton()->getStatsManager())
         stats_manager = Simulator::getSingleton()->getStatsManager();
   }
   if (stats_manager)
      stats_manager->registerMetric(new StatsMetric<T>(objectName, index, metricName, metric));
}
...
class StatHist
{
 ...
 public:
   StatHist() : n(0), s(0), s2(0), min(0), max(0)
   {
