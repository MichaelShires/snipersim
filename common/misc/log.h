...
#define __LOG_PRINT(err, file, line, ...)                                                                              \
   {                                                                                                                   \
      if (unlikely(Log::getSingleton()->isLoggingEnabled()) || err != Log::None) {                                     \
         ...
         if (err != Log::None || Log::getSingleton()->isEnabled(module.c_str())) {                                     \
...
#define LOG_PRINT_ERROR(...)                                                                                           \
   {                                                                                                                   \
      ...
      throw SimulationException("Fatal Error reported via LOG_PRINT_ERROR");                                          \
   }
...
#define LOG_ASSERT_ERROR(expr, ...)                                                                                    \
   {                                                                                                                   \
      if (!(expr)) {                                                                                                   \
         _LOG_PRINT(Log::Error, __VA_ARGS__);                                                                          \
         throw SimulationException("Assertion failed: " #expr);                                                        \
      }                                                                                                                \
   }
...
{
 public:
   FunctionTracer(const char *file, int line, const char *fn) : m_file(file), m_line(line), m_fn(fn)
   {
