...
void SpinLoopDetector::commitBCT(uint64_t eip)
{
   ...
   if (entry == m_sdt.end()) {
      ...
      m_sdt.push_back(
          SdtEntry(eip, m_sdt_nextid, core->getInstructionCount(), core->getPerformanceModel()->getElapsedTime()));
      ...
      if (spinning) {
         ...
         core->updateSpinCount(
             ...
             // in this exact spin loop. But, when spinning for a long time, the earlier instructions will be spin loops
             // as well so if spin count is large (which is when we care) it should be reasonably accurate.
...
void SpinLoopDetector::commitRegisterWrite(reg_t reg, uint64_t old_value, uint64_t value)
{
   ...
   if (m_rub.count(reg) == 0) {
      ...
      // ``If the register is already in the RUB, then the processor compares its new value to its current value in the
      // RUB.''
