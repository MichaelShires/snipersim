...
template <class T> MovingAverage<T>::MovingAverage(UInt32 max_window_size) : m_max_window_size(max_window_size)
{
...
template <class T> MovingAverage<T>::~MovingAverage()
{
}

template <class T> void MovingAverage<T>::addToWindow(T next_num)
{
...
template <class T> class MovingArithmeticMean : public MovingAverage<T>
{
 private:
   T sum;

 public:
   MovingArithmeticMean(UInt32 max_window_size) : MovingAverage<T>(max_window_size), sum(SubsecondTime::Zero())
   {
   }
...
template <class T> class MovingGeometricMean : public MovingAverage<T>
{
 public:
   MovingGeometricMean(UInt32 max_window_size) : MovingAverage<T>(max_window_size)
   {
   }
...
template <class T> class MovingMedian : public MovingAverage<T>
{
 public:
...
template <class T> MovingAverage<T> *MovingAverage<T>::createAvgType(AvgType_t avg_type, UInt32 max_window_size)
{
   switch (avg_type) {
