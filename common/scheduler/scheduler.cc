...
Scheduler::Scheduler(ThreadManager *thread_manager) : m_thread_manager(thread_manager)
{
}
...
void Scheduler::printMapping()
{
   ...
   for (core_id_t core_id = 0; core_id < (core_id_t)Sim()->getConfig()->getApplicationCores(); core_id++) {
      if (Sim()->getCoreManager()->getCoreFromID(core_id)->getState() != Core::IDLE) {
         char state;
         switch (Sim()->getThreadManager()->getThreadState(
             Sim()->getCoreManager()->getCoreFromID(core_id)->getThread()->getId()))
         {
         ...
         std::cout << "( t" << Sim()->getCoreManager()->getCoreFromID(core_id)->getThread()->getId() << "@c" << core_id
                   << "[" << state << "])";
      }
      else {
