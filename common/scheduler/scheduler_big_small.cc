...
SchedulerBigSmall::SchedulerBigSmall(ThreadManager *thread_manager)
    : SchedulerPinnedBase(thread_manager, SubsecondTime::NS(Sim()->getCfg()->getInt("scheduler/big_small/quantum"))),
      m_debug_output(Sim()->getCfg()->getBool("scheduler/big_small/debug")), m_last_reshuffle(SubsecondTime::Zero()),
      m_rng(rng_seed(42))
{
   // Figure out big and small cores, and create affinity masks for the set of big cores and the set of small cores,
   // respectively
...
void SchedulerBigSmall::periodic(SubsecondTime time)
{
   ...
   if (time > m_last_reshuffle + m_quantum) {
      ...
      for (UInt64 i = 0; i < std::min((UInt64)m_num_big_cores, (UInt64)Sim()->getThreadManager()->getNumThreads()); ++i) {
         pickBigThread();
      }
