...
class SchedulerPinnedBase : public SchedulerDynamic
{
 public:
   ...
   virtual bool threadSetAffinity(thread_id_t calling_thread_id, thread_id_t thread_id, size_t cpusetsize,
    ...
    public:
      ThreadInfo()
          : m_has_affinity(false), m_explicit_affinity(false),
            m_core_affinity(Sim()->getConfig()->getApplicationCores(), false), m_core_running(INVALID_CORE_ID),
            m_last_scheduled_in(SubsecondTime::Zero()), m_last_scheduled_out(SubsecondTime::Zero())
      {
      }
      /* affinity */
      void clearAffinity()
