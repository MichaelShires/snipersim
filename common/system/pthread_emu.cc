...
const char *pthread_names[] = {"pthread_mutex_lock",  "pthread_mutex_trylock", "pthread_mutex_unlock",
...
void updateState(Core *core, state_t state, SubsecondTime delay)
{
   if (trace_fp) {
      ...
      fprintf(trace_fp, "%u %" PRIu64 " %u\n", core->getId(),
              (core->getPerformanceModel()->getElapsedTime() + delay).getNS(), state);
   }
}
   ...
   if (!pthread_stats_added) {
      ...
      __attribute__((unused)) int rc =
          posix_memalign((void **)&pthread_counters, 64,
                         pthread_counters_size); // Align by cache line size to prevent thread contention
      ...
      for (uint32_t c = 0; c < num_cores; c++) {
         for (int e = PTHREAD_MUTEX_LOCK; e < PTHREAD_ENUM_LAST; e++) {
            ...
            registerStatsMetric("pthread", c, String(pthread_names[e]) + "_delay_sync",
                                &(pthread_counters[c].pthread_total_delay_sync[e]));
            registerStatsMetric("pthread", c, String(pthread_names[e]) + "_delay_mem",
         ...
         registerStatsMetric("pthread", c, "pthread_mutex_unlock_contended",
...
IntPtr MutexLock(pthread_mutex_t *mux)
{
   ...
   MemoryResult lat = core->accessMemory(Core::NONE, Core::READ_EX, (IntPtr)mux, (char *)&_mux, sizeof(pthread_mutex_t),
   ...
   if (delay > SubsecondTime::Zero()) { /* Assume in the uncontended case, nothing (not the (system) network, nor the
                                           MCPs SyncServer) adds any delay */
      /* Model the lock addw(hb->spinlock) inside the futex_wake syscall */
      lat1 = core->accessMemory(Core::NONE, Core::READ_EX, (IntPtr)futexHbAddress(mux), NULL, sizeof(UInt32),
                                Core::MEM_MODELED_FENCED);
      pthread_counters[core->getId()].pthread_mutex_lock_contended++;
   }

   /* Delay and lat will be pushed as dynamic instructions, but have not been processed yet so we need to tell
    * updateState to add them to core->getCycleCount(). */
...
IntPtr MutexTrylock(pthread_mutex_t *mux)
{
   ...
   pthreadCount(PTHREAD_MUTEX_TRYLOCK, core, res == SubsecondTime::MaxTime() ? SubsecondTime::Zero() : res,
                lat.latency);
...
IntPtr MutexUnlock(pthread_mutex_t *mux)
{
   ...
   if (delay > SubsecondTime::Zero()) {
      ...
      //   But we can't request the latency until we've contacted the server (which already releases the lock) to tell
      //   us whether it's contended Also, no-one is currently spinning on this (and keeping the line in shared state)
      //   -- in fact, we may have even been the last ones to have used it in our matching pthread_mutex_lock call
      lat1 = core->accessMemory(Core::NONE, Core::READ_EX, (IntPtr)futexHbAddress(mux), NULL, sizeof(UInt32),
                                Core::MEM_MODELED_FENCED);
...
IntPtr CondWait(pthread_cond_t *cond, pthread_mutex_t *mutex)
{
   ...
   MemoryResult lat2 = core->accessMemory(Core::NONE, Core::READ_EX, (IntPtr)mutex, (char *)&_mutex,
                                          sizeof(pthread_mutex_t), Core::MEM_MODELED_FENCED);
   MemoryResult lat1 = core->accessMemory(Core::NONE, Core::READ_EX, (IntPtr)cond, (char *)&_cond,
...
IntPtr CondSignal(pthread_cond_t *cond)
{
   ...
   MemoryResult lat = core->accessMemory(Core::NONE, Core::READ_EX, (IntPtr)cond, (char *)&_cond,
                                         sizeof(pthread_cond_t), Core::MEM_MODELED_FENCED);
...
IntPtr BarrierWait(pthread_barrier_t *barrier)
{
   ...
   /* Use READ_EX rather than READ since a real pthread_barrier_wait() would write to barrier, so we need the lines in M
      state. Also use MEM_MODELED_FENCED since there is a lock cmpxchg instruction in the implementation of
      pthread_barrier_wait */
   MemoryResult lat = core->accessMemory(Core::NONE, Core::READ_EX, (IntPtr)barrier, (char *)&barrier_buf,
