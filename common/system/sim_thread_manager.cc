...
SimThreadManager::SimThreadManager(SimulationContext *context)
    : m_context(context), m_sim_threads(NULL), m_core_threads(NULL), m_active_threads(0), 
      m_config(context->getConfig()), m_transport(context->getTransport())
{
}

SimThreadManager::~SimThreadManager()
{
   LOG_ASSERT_WARNING(m_active_threads == 0, "Threads still active when SimThreadManager exits.");
}

void SimThreadManager::spawnSimThreads()
{
   UInt32 num_cores = m_config->getTotalCores();
...
void SimThreadManager::quitSimThreads()
{
   ...
   Transport::Node *global_node = m_transport->getGlobalNode();
   ...
   for (core_id_t core_id = 0; core_id < (core_id_t)m_config->getTotalCores(); core_id++) {
#ifdef ENABLE_PERF_MODEL_OWN_THREAD
