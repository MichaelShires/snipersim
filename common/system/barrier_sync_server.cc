...
BarrierSyncServer::BarrierSyncServer(SimulationContext *context)
    : m_local_clock_list(context->getConfig()->getApplicationCores(), SubsecondTime::Zero()),
      m_barrier_acquire_list(context->getConfig()->getApplicationCores(), false),
      m_core_cond(context->getConfig()->getApplicationCores(), NULL),
      m_core_group(context->getConfig()->getApplicationCores(), INVALID_CORE_ID),
      m_core_thread(context->getConfig()->getApplicationCores(), INVALID_THREAD_ID), m_global_time(SubsecondTime::Zero()),
      m_fastforward(false), m_disable(false), m_context(context), m_cfg(context->getConfigFile()),
      m_config(context->getConfig()), m_hooks_manager(context->getHooksManager()),
      m_thread_manager(NULL), m_core_manager(NULL), m_simulator(context->getSimulator())
{
   try {
      auto quantum = m_cfg->getInt("clock_skew_minimization/barrier/quantum");
      if (quantum != 0) {
   ...
   for (core_id_t core_id = 0; core_id < (core_id_t)m_config->getApplicationCores(); ++core_id)
   ...
   m_hooks_manager->registerHook(HookType::HOOK_THREAD_EXIT, BarrierSyncServer::hookThreadExit, (UInt64)this,
                                          HooksManager::ORDER_NOTIFY_POST);
   m_hooks_manager->registerHook(HookType::HOOK_THREAD_STALL, BarrierSyncServer::hookThreadStall, (UInt64)this,
                                          HooksManager::ORDER_NOTIFY_POST);
   m_hooks_manager->registerHook(HookType::HOOK_THREAD_MIGRATE, BarrierSyncServer::hookThreadMigrate,
...
void BarrierSyncServer::synchronize(core_id_t core_id, SubsecondTime time)
{
   ...
   if (m_fastforward)
      master_core_id = core_id; // In fast-forward, the SMT performance model in not active so every core (HW context)
                                // calls into the barrier
   else
      master_core_id = m_core_group[core_id] == INVALID_CORE_ID ? core_id : m_core_group[core_id];
   Core *master_core = m_core_manager->getCoreFromID(core_id);
   thread_id_t thread_me = core->getThread()->getId();

   CLOG("barrier", "Core %d entry (master core %d, thread %d, ffwd %d)", core_id, master_core_id, thread_me,
        m_fastforward);
   LOG_PRINT("Received 'SIM_BARRIER_WAIT' from Core(%i), Time(%s)", core_id, itostr(time).c_str());

   LOG_ASSERT_ERROR(core->getState() == Core::RUNNING || core->getState() == Core::INITIALIZING,
                    "Core(%i) is not running or initializing at time(%s)", core_id, itostr(time).c_str());
   LOG_ASSERT_ERROR(m_barrier_acquire_list[master_core_id] == false,
                    "Core(%i) or its sibling is already in the barrier (this is thread %d, we have thread %d)",
                    master_core_id, thread_me, m_core_thread[master_core_id]);

   if (time < m_next_barrier_time && !m_fastforward) {
      LOG_PRINT("Sent 'SIM_BARRIER_RELEASE' immediately time(%s), m_next_barrier_time(%s)", itostr(time).c_str(),
                itostr(m_next_barrier_time).c_str());
      // LOG_PRINT_WARNING("core_id(%i), local_clock(%llu), m_next_barrier_time(%llu), m_barrier_interval(%llu)",
      // core_id, time, m_next_barrier_time, m_barrier_interval);
   ...
   if (mustWait)
      m_core_cond[master_core_id]->wait(m_lock);
   else
...
bool BarrierSyncServer::isCoreRunning(core_id_t core_id, bool siblings)
{
   ...
   if (core->getState() == Core::RUNNING) {
      ...
      if (m_thread_manager->isThreadRunning(core->getThread()->getId()))
         return true;
   }

   if (siblings && !m_fastforward) {
      for (core_id_t sibling_core_id = 0; sibling_core_id < (core_id_t)m_config->getApplicationCores();
           sibling_core_id++)
      {
...
bool BarrierSyncServer::isBarrierReached()
{
   ...
   for (core_id_t core_id = 0; core_id < (core_id_t)m_config->getApplicationCores(); core_id++) {
      // In fastforward mode, it's enough that a core is waiting. In detailed mode, it needs to have advanced up to the
      // predefined barrier time
      if (m_fastforward) {
...
bool BarrierSyncServer::barrierRelease(thread_id_t caller_id, bool continue_until_release)
{
   ...
   while (!core_resumed) {
      ...
      m_hooks_manager->callHooks(HookType::HOOK_PERIODIC,
      ...
      if (continue_until_release) {
         ...
         if (m_thread_manager->anyThreadRunning())
            return false;
         else
            LOG_ASSERT_ERROR(m_simulator->getSyscallServer()->getNextTimeout(m_global_time) < SubsecondTime::MaxTime(),
      ...
      for (core_id_t core_id = 0; core_id < (core_id_t)m_config->getApplicationCores(); core_id++) {
         if (m_local_clock_list[core_id] < m_next_barrier_time) {
            ...
            if (m_barrier_acquire_list[core_id] == true) {
               // Core *core = m_core_manager->getCoreFromID(core_id);
               // LOG_ASSERT_ERROR(core->getState() == Core::RUNNING || core->getState() == Core::INITIALIZING, "(%i)
               // has acquired barrier, local_clock(%s), m_next_barrier_time(%s), but not initializing or running",
               // core_id, itostr(m_local_clock_list[core_id]).c_str(), itostr(m_next_barrier_time).c_str());
   ...
   doRelease(m_fastforward ? -1 : m_config->getNumHostCores());
...
void BarrierSyncServer::setGroup(core_id_t core_id, core_id_t master_core_id)
{
   ...
   if (master_core_id != INVALID_CORE_ID)
      LOG_ASSERT_ERROR(m_barrier_acquire_list[core_id] == false,
...
bool BarrierSyncServer::onlyMainCoreRunning(void)
{
   ScopedLock sl(m_lock);
