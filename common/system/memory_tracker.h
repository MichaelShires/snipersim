...
class MemoryTracker
{
 public:
   ...
   class RoutineTracerThread : public ::RoutineTracerThread
   {
    public:
      RoutineTracerThread(Thread *thread, HooksManager *hooks_manager, MagicServer *magic_server) 
          : ::RoutineTracerThread(thread, hooks_manager, magic_server)
      {
      }
      const CallStack &getCallsiteStack() const
      {
         return m_callsite_stack;
      }
   ...
   {
    public:
      RoutineTracer(SimulationContext *context);
      virtual ~RoutineTracer();

      virtual RoutineTracerThread *getThreadHandler(Thread *thread)
      {
         return new RoutineTracerThread(thread, m_hooks_manager, m_magic_server);
      }
   ...
   MemoryTracker(Config *config, ThreadManager *thread_manager, ::RoutineTracer *routine_tracer);
   ~MemoryTracker();
 ...
 private:
   struct AllocationSite
   {
      AllocationSite()
          : num_allocations(0), total_size(0), total_loads(0), total_stores(0),
            hit_where_load(HitWhere::NUM_HITWHERES, 0), hit_where_store(HitWhere::NUM_HITWHERES, 0)
      {
      }
   ...
   ::RoutineTracer *m_routine_tracer;
   ...
   void ce_notify_evict(bool on_roi_end, UInt64 owner, UInt64 evictor, CacheBlockInfo::BitsUsedType bits_used,
                        UInt32 bits_total);
   ...
   static void __ce_notify_evict(UInt64 user, bool on_roi_end, UInt64 owner, UInt64 evictor,
