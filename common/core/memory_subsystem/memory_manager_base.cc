...
#include "simulator.h"
MemoryManagerBase *MemoryManagerBase::createMMU(String protocol_type, Core *core, Network *network,
                                               ShmemPerfModel *shmem_perf_model, SimulationContext *context)
{
   CachingProtocol_t caching_protocol_type = parseProtocolType(protocol_type);

   switch (caching_protocol_type) {
   case PARAMETRIC_DRAM_DIRECTORY_MSI:
      return new ParametricDramDirectoryMSI::MemoryManager(core, network, shmem_perf_model, context);

   case FAST_NEHALEM:
      return new FastNehalem::MemoryManager(core, network, shmem_perf_model, context);

   default:
      LOG_PRINT_ERROR("Unsupported Caching Protocol Type: %s", protocol_type.c_str());
      return NULL;
   }
}

MemoryManagerBase::CachingProtocol_t MemoryManagerBase::parseProtocolType(String &protocol_type)
{
...
std::vector<core_id_t> MemoryManagerBase::getCoreListWithMemoryControllers()
{
   ...
   core_count = getConfigLegacy()->getApplicationCores();
   try {
      num_memory_controllers = getConfig()->getInt("perf_model/dram/num_controllers");
      UInt32 smt_cores = getConfig()->getInt("perf_model/core/logical_cpus");
      memory_controllers_interleaving = getConfig()->getInt("perf_model/dram/controllers_interleaving") * smt_cores;
      memory_controller_positions_from_cfg_file = getConfig()->getString("perf_model/dram/controller_positions");
   } catch (...) {
   ...
   if (num_memory_controllers != -1) {
      ...
      LOG_ASSERT_ERROR((core_list_from_cfg_file.size() == 0) ||
                       ...
                       "num_memory_controllers(%i), num_controller_positions specified(%i)", num_memory_controllers,
                       core_list_from_cfg_file.size());
      ...
      if (core_list_from_cfg_file.size() > 0) {
         ...
         try {
            l_models_memory_1 = NetworkModel::parseNetworkType(getConfig()->getString("network/memory_model_1"));
         } catch (...) {
            LOG_PRINT_ERROR("Exception while reading network model types.");
         }

         std::pair<bool, std::vector<core_id_t>> core_list_with_memory_controllers_1 =
             NetworkModel::computeMemoryControllerPositions(l_models_memory_1, num_memory_controllers, core_count);
      ...
      if (memory_controllers_interleaving) {
         num_memory_controllers =
             (core_count + memory_controllers_interleaving - 1) / memory_controllers_interleaving; // Round up
...
void MemoryManagerBase::parseMemoryControllerList(String &memory_controller_positions,
...
void MemoryManagerBase::printCoreListWithMemoryControllers(std::vector<core_id_t> &core_list_with_memory_controllers)
{
   std::ostringstream core_list;
   for (std::vector<core_id_t>::iterator it = core_list_with_memory_controllers.begin();
