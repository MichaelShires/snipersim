...
{
   static_assert(CacheBlockInfo::NUM_OPTIONS == sizeof(CacheBlockInfo::option_names) / sizeof(char *),
...
{
   UInt64 first = offset >> BitsUsedOffset, last = (offset + size - 1) >> BitsUsedOffset,
          first_mask = (1ull << first) - 1, last_mask = (1ull << (last + 1)) - 1, usage_mask = last_mask & ~first_mask;
