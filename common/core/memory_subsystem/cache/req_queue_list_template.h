...
template <class T_Req> void ReqQueueListTemplate<T_Req>::enqueue(IntPtr address, T_Req *shmem_req)
{
...
template <class T_Req> T_Req *ReqQueueListTemplate<T_Req>::dequeue(IntPtr address)
{
   LOG_ASSERT_ERROR(m_req_queue_list.count(address) != 0, "Could not find a request with address(0x%x)", address);
...
template <class T_Req> T_Req *ReqQueueListTemplate<T_Req>::front(IntPtr address)
{
   LOG_ASSERT_ERROR(m_req_queue_list.count(address) != 0, "Could not find a request with address(0x%x)", address);
...
template <class T_Req> T_Req *ReqQueueListTemplate<T_Req>::back(IntPtr address)
{
   LOG_ASSERT_ERROR(m_req_queue_list.count(address) != 0, "Could not find a request with address(0x%x)", address);
...
template <class T_Req> UInt32 ReqQueueListTemplate<T_Req>::size(IntPtr address)
{
   if (m_req_queue_list.count(address) == 0)
      return 0;
   else
...
template <class T_Req> bool ReqQueueListTemplate<T_Req>::empty(IntPtr address)
{
   if (m_req_queue_list.count(address) == 0)
      return true;
   else
