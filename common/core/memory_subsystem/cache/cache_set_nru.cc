...
CacheSetNRU::CacheSetNRU(CacheBase::cache_t cache_type, UInt32 associativity, UInt32 blocksize)
...
UInt32 CacheSetNRU::getReplacementIndex(CacheCntlr *cntlr)
{
   ...
   for (UInt32 i = 0; i < m_associativity; i++) {
      if (!m_cache_block_info_array[i]->isValid()) {
         ...
         // invalid line to replace Mark our newly-inserted line as recently used
         ...
         // If there are none, we'll ignore the LRU bit below and select the first entry with isValidReplacement() as
         // the victim
         have_zero_bit = true;
      }
   }
...
static ComponentRegistrar<CacheSet, String, core_id_t, CacheBase::cache_t, UInt32, UInt32, CacheSetInfo*> 
   nru_registrar("nru", [](String cfgname, core_id_t core_id, CacheBase::cache_t cache_type, UInt32 associativity, UInt32 blocksize, CacheSetInfo *set_info) -> CacheSet* {
