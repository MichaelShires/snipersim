...
std::vector<IntPtr> GhbPrefetcher::getNextAddress(IntPtr currentAddress, core_id_t core_id)
{
   ...
   UInt32 i = 0;
   while (i < m_tableSize && m_ghbTable[i].delta != INVALID_DELTA && m_ghbTable[i].delta != delta)
      ++i;

   if (i != m_tableSize && m_ghbTable[i].delta == delta &&
       m_ghbTable[i].generation ==
           m_ghb[m_ghbTable[i].ghbIndex].generation) // check if the table still points to the current GHB 'generation'
   {
      UInt32 width = 0;
   ...
   if (prevDelta != INVALID_DELTA) {
      i = 0;
      while (i < m_tableSize && m_ghbTable[i].delta != INVALID_DELTA && m_ghbTable[i].delta != prevDelta)
         ++i;

      if (i != m_tableSize && m_ghbTable[i].delta == prevDelta) { // update existing entry
