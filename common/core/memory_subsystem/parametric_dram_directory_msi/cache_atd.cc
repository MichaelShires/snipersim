...
ATD::ATD(String name, String configName, core_id_t core_id, UInt32 num_sets, UInt32 associativity,
    ...
    : m_cache_base(name, num_sets, associativity, cache_block_size, hash_function), m_sets(), loads(0), stores(0),
      load_misses(0), store_misses(0), loads_constructive(0), stores_constructive(0), loads_destructive(0),
      stores_destructive(0)
{
   ...
   if (sampling == "full") {
      for (UInt64 set_index = 0; set_index < num_sets; ++set_index) {
         m_sets[set_index] = CacheSet::createCacheSet(name, core_id, replacement_policy, CacheBase::PR_L1_CACHE,
      ...
      for (UInt64 set_index = 1; set_index < num_sets - 1; set_index <<= 1) {
         m_sets[set_index + 1] = CacheSet::createCacheSet(name, core_id, replacement_policy, CacheBase::PR_L1_CACHE,
                                                          associativity, 0, m_set_info);
      }
   }
      ...
      LOG_ASSERT_ERROR(num_atds <= num_sets, "Cannot sample more sets (%d) than the total number of sets (%d)",
                       num_atds, num_sets);
