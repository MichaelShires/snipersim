...
SimplePrefetcher::SimplePrefetcher(String configName, core_id_t _core_id, UInt32 _shared_cores)
    : core_id(_core_id), shared_cores(_shared_cores),
      ...
      flows_per_core(
          Sim()->getCfg()->getBoolArray("perf_model/" + configName + "/prefetcher/simple/flows_per_core", core_id)),
      num_prefetches(
          Sim()->getCfg()->getIntArray("perf_model/" + configName + "/prefetcher/simple/num_prefetches", core_id)),
      stop_at_page(Sim()->getCfg()->getBoolArray(
          "perf_model/" + configName + "/prefetcher/simple/stop_at_page_boundary", core_id)),
      n_flow_next(0), m_prev_address(flows_per_core ? shared_cores : 1)
{
   for (UInt32 idx = 0; idx < (flows_per_core ? shared_cores : 1); ++idx)
      m_prev_address.at(idx).resize(n_flows);
}

std::vector<IntPtr> SimplePrefetcher::getNextAddress(IntPtr current_address, core_id_t _core_id)
{
