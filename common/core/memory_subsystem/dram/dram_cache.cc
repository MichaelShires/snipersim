...
DramCache::DramCache(MemoryManagerBase *memory_manager, ShmemPerfModel *shmem_perf_model,
                     AddressHomeLookup *home_lookup, UInt32 cache_block_size, DramCntlrInterface *dram_cntlr)
    : DramCntlrInterface(memory_manager, shmem_perf_model, cache_block_size),
      m_core_id(memory_manager->getCore()->getId()), m_cache_block_size(cache_block_size),
      m_data_access_time(
          SubsecondTime::NS(Sim()->getCfg()->getIntArray("perf_model/dram/cache/data_access_time", m_core_id))),
      m_tags_access_time(
          SubsecondTime::NS(Sim()->getCfg()->getIntArray("perf_model/dram/cache/tags_access_time", m_core_id))),
      m_data_array_bandwidth(8 * Sim()->getCfg()->getFloat("perf_model/dram/cache/bandwidth")),
      m_home_lookup(home_lookup), m_dram_cntlr(dram_cntlr), m_queue_model(NULL), m_prefetcher(NULL),
      m_prefetch_mshr("dram-cache.prefetch-mshr", m_core_id, 16), m_reads(0), m_writes(0), m_read_misses(0),
      m_write_misses(0), m_hits_prefetch(0), m_prefetches(0), m_prefetch_mshr_delay(SubsecondTime::Zero())
{
   ...
   LOG_ASSERT_ERROR(k_KILO * cache_size == num_sets * associativity * m_cache_block_size,
   ...
   m_cache = new Cache(
       "dram-cache", "perf_model/dram/cache", m_core_id, num_sets, associativity, m_cache_block_size,
       Sim()->getCfg()->getStringArray("perf_model/dram/cache/replacement_policy", m_core_id), CacheBase::PR_L1_CACHE,
   ...
   if (Sim()->getCfg()->getBool("perf_model/dram/cache/queue_model/enabled")) {
      ...
      m_queue_model =
          QueueModel::create("dram-cache-queue", m_core_id, queue_model_type,
                             m_data_array_bandwidth.getRoundedLatency(8 * m_cache_block_size)); // bytes to bits
   }

   m_prefetcher = Prefetcher::createPrefetcher(Sim()->getCfg()->getString("perf_model/dram/cache/prefetcher"),
...
boost::tuple<SubsecondTime, HitWhere::where_t> DramCache::putDataToDram(IntPtr address, core_id_t requester,
...
std::pair<bool, SubsecondTime> DramCache::doAccess(Cache::access_t access, IntPtr address, core_id_t requester,
                                                   Byte *data_buf, SubsecondTime now, ShmemPerf *perf)
{
   ...
   if (block_info) {
      ...
      if (access == Cache::LOAD) {
         ...
         boost::tie(dram_latency, hit_where) =
             m_dram_cntlr->getDataFromDram(address, requester, data_buf, now + latency, perf);
...
void DramCache::insertLine(Cache::access_t access, IntPtr address, core_id_t requester, Byte *data_buf,
                           SubsecondTime now)
{
   bool eviction;
   ...
   m_cache->insertSingleLine(address, data_buf, &eviction, &evict_address, &evict_block_info, evict_buf, now);
...
SubsecondTime DramCache::accessDataArray(Cache::access_t access, core_id_t requester, SubsecondTime t_start,
                                         ShmemPerf *perf)
{
   ...
   if (!cache_hit || (m_prefetch_on_prefetch_hit && prefetch_hit)) {
      for (std::vector<IntPtr>::iterator it = prefetchList.begin(); it != prefetchList.end(); ++it) {
         ...
         if (!m_cache->peekSingleLine(prefetch_address)) {
            ...
            boost::tie(dram_latency, hit_where) =
                m_dram_cntlr->getDataFromDram(prefetch_address, m_core_id, data_buf, t_issue, NULL);
