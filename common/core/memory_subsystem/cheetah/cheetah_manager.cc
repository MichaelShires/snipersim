...
CheetahManager::CheetahManager(core_id_t core_id)
    : m_min_bits(Sim()->getCfg()->getInt("core/cheetah/min_size_bits")),
      ...
      m_max_bits_global(Sim()->getCfg()->getInt("core/cheetah/max_size_bits_global")), m_address_buffer_size(0)
{
   LOG_ASSERT_ERROR(m_min_bits >= CheetahModel::getMinSize(), "cheetah/min_size_bits (%d) must be >= %d", m_min_bits,
                    CheetahModel::getMinSize());
   LOG_ASSERT_ERROR(m_max_bits_local >= CheetahModel::getMinSize(), "cheetah/max_size_bits_local (%d) must be >= %d",
                    m_max_bits_local, CheetahModel::getMinSize());
   LOG_ASSERT_ERROR(m_max_bits_global >= CheetahModel::getMinSize(), "cheetah/max_size_bits_global (%d) must be >= %d",
   ...
   if ((core_id & 1) == 0)
      s_cheetah_models[CHEETAH_BY2].push_back(new CheetahModel(true, m_min_bits, m_max_bits_local));
   if ((core_id & 3) == 0)
      s_cheetah_models[CHEETAH_BY4].push_back(new CheetahModel(true, m_min_bits, m_max_bits_local));
   if ((core_id & 7) == 0)
      s_cheetah_models[CHEETAH_BY8].push_back(new CheetahModel(true, m_min_bits, m_max_bits_local));
   if (core_id == 0)
      s_cheetah_models[CHEETAH_GLOBAL].push_back(new CheetahModel(true, m_min_bits, m_max_bits_global));
...
CheetahManager::CheetahStats::CheetahStats(UInt32 min_bits, UInt32 max_bits_local, UInt32 max_bits_global)
    : m_min_bits(min_bits), m_max_bits_local(max_bits_local), m_max_bits_global(max_bits_global)
{
   ...
   Sim()->getHooksManager()->registerHook(HookType::HOOK_PRE_STAT_WRITE, hook_update, (UInt64)this,
