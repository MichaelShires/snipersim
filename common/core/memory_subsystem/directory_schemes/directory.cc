...
Directory::Directory(core_id_t core_id, String directory_type_str, UInt32 num_entries, UInt32 max_hw_sharers,
                     UInt32 max_num_sharers)
    : m_num_entries(num_entries), m_num_entries_allocated(0), m_max_hw_sharers(max_hw_sharers),
      m_use_max_hw_sharers(max_hw_sharers), // Value to pass through to DirectoryEntry::addSharer
      m_max_num_sharers(max_num_sharers), m_limitless_software_trap_penalty(SubsecondTime::Zero())
{
   ...
   if (m_directory_type == LIMITLESS) {
      try {
         m_limitless_software_trap_penalty =
             SubsecondTime::NS() * Sim()->getCfg()->getInt("perf_model/dram_directory/limitless/software_trap_penalty");
      } catch (...) {
...
void Directory::setDirectoryEntry(UInt32 entry_num, DirectoryEntry *directory_entry)
{
   m_directory_entry_list[entry_num] = directory_entry;
}

Directory::DirectoryType Directory::parseDirectoryType(String directory_type_str)
{
...
template <class DirectorySharers> DirectoryEntry *Directory::createDirectoryEntrySized()
{
   ...
   case LIMITLESS:
      return new DirectoryEntryLimitless<DirectorySharers>(m_max_hw_sharers, m_max_num_sharers,
                                                           m_limitless_software_trap_penalty);
