...
template <long Size> class DirectorySharersBitset : public std::bitset<Size>
{
 public:
...
class DirectoryEntry
{
 ...
 public:
   DirectoryEntry() : m_address(INVALID_ADDRESS), m_directory_block_info(), m_owner_id(INVALID_CORE_ID)
   {
   }
...
template <class DirectorySharers> class DirectoryEntrySized : public DirectoryEntry
{
 protected:
   DirectorySharers m_sharers;

 public:
   DirectoryEntrySized(UInt32 max_hw_sharers, UInt32 max_num_sharers) : DirectoryEntry(), m_sharers(max_num_sharers)
   {
   }

   virtual UInt32 getNumSharers()
   {
      return m_sharers.count();
   }
