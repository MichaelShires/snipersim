...
class Dram : public CacheBase
{
 ...
 public:
   Dram(Core *core, String name, UInt64 latency) : m_latency(core->getDvfsDomain(), latency)
   {
      m_reads = m_writes = 0;
...
template <UInt32 assoc> class CacheSet
{
 private:
   IntPtr m_tags[assoc];
   UInt64 m_lru[assoc];
...
template <UInt32 assoc, UInt32 size_kb> class Cache : public CacheBase
{
 private:
 ...
 public:
   Cache(Core *core, String name, MemComponent::component_t mem_component, UInt64 latency, CacheBase *next_level)
       : m_mem_component(mem_component), m_latency(core->getDvfsDomain(), latency), m_next_level(next_level),
         m_num_sets(size_kb * 1024 / 64 / assoc), m_sets_mask(m_num_sets - 1), m_sets(m_num_sets)
   {
...
template <UInt32 assoc, UInt32 size_kb> class CacheLocked : public Cache<assoc, size_kb>
{
 private:
   Lock lock;
