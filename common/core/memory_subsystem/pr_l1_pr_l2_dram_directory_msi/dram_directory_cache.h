...
class DramDirectoryCache
{
 private:
   ...
   {
      return m_log_num_sets;
   }

 public:
   DramDirectoryCache(core_id_t core_id, String directory_type_str, UInt32 total_entries, UInt32 associativity,
                      UInt32 cache_block_size, UInt32 max_hw_sharers, UInt32 max_num_sharers,
                      ComponentLatency dram_directory_cache_access_time, ShmemPerfModel *shmem_perf_model);
   ~DramDirectoryCache();
   ...
   {
      return m_directory->getMaxHwSharers();
   }
};
} // namespace PrL1PrL2DramDirectoryMSI
