...
#define MYLOG(...)                                                                                                     \
   {                                                                                                                   \
      ScopedLock l(iolock);                                                                                            \
      fflush(stdout);                                                                                                  \
      printf("[%s] %d%cdd %-25s@%3u: ", itostr(getShmemPerfModel()->getElapsedTime()).c_str(),                         \
             getMemoryManager()->getCore()->getId(), Sim()->getCoreManager()->amiUserThread() ? '^' : '_',             \
...
DramDirectoryCntlr::DramDirectoryCntlr(core_id_t core_id, MemoryManagerBase *memory_manager,
    ...
    : m_memory_manager(memory_manager), m_dram_controller_home_lookup(dram_controller_home_lookup),
      m_nuca_cache(nuca_cache), m_core_id(core_id), m_cache_block_size(cache_block_size),
      m_shmem_perf_model(shmem_perf_model), forward(0), forward_failed(0)
{
   m_dram_directory_cache =
       new DramDirectoryCache(core_id, dram_directory_type_str, dram_directory_total_entries,
                              dram_directory_associativity, cache_block_size, dram_directory_max_hw_sharers,
                              dram_directory_max_num_sharers, dram_directory_cache_access_time, m_shmem_perf_model);
   m_dram_directory_req_queue_list = new ReqQueueList();
   for (DirectoryState::dstate_t state = DirectoryState::dstate_t(0); state < DirectoryState::NUM_DIRECTORY_STATES;
...
void DramDirectoryCntlr::handleMsgFromL2Cache(core_id_t sender, ShmemMsg *shmem_msg)
{
   ...
   // This is just for modeling the TD lookup time (this is the only place where we call getDirectoryEntry with
   // modeled==true), elsewhere we assume outstanding requests are stored in a fast MSHR-like structure
...
void DramDirectoryCntlr::handleMsgFromDRAM(core_id_t sender, ShmemMsg *shmem_msg)
{
   MYLOG("Start");
   ShmemMsg::msg_t shmem_msg_type = shmem_msg->getMsgType();
...
DirectoryEntry *DramDirectoryCntlr::processDirectoryEntryAllocationReq(ShmemReq *shmem_req)
{
   ...
   ShmemMsg nullify_msg(ShmemMsg::NULLIFY_REQ, MemComponent::TAG_DIR, MemComponent::TAG_DIR, requester,
                        replaced_address, NULL, 0, &m_dummy_shmem_perf);
...
void DramDirectoryCntlr::processNullifyReq(ShmemReq *shmem_req)
{
   ...
   case DirectoryState::EXCLUSIVE:
   case DirectoryState::MODIFIED:
      getMemoryManager()->sendMsg(ShmemMsg::FLUSH_REQ, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
                                  requester /* requester */, directory_entry->getOwner() /* receiver */, address, NULL,
                                  0, HitWhere::UNKNOWN, &m_dummy_shmem_perf, ShmemPerfModel::_SIM_THREAD);
      break;
...

   {
      ...
      if (sharers_list_pair.first == true) {
         ...
         getMemoryManager()->broadcastMsg(ShmemMsg::INV_REQ, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
         ...
         for (UInt32 i = 0; i < sharers_list_pair.second.size(); i++) {
            getMemoryManager()->sendMsg(ShmemMsg::INV_REQ, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
                                        requester /* requester */, sharers_list_pair.second[i] /* receiver */, address,
                                        NULL, 0, HitWhere::UNKNOWN, &m_dummy_shmem_perf, ShmemPerfModel::_SIM_THREAD);
         }
      }
   } break;
...
void DramDirectoryCntlr::processExReqFromL2Cache(ShmemReq *shmem_req, Byte *cached_data_buf)
{
   IntPtr address = shmem_req->getShmemMsg()->getAddress();
   core_id_t requester = shmem_req->getShmemMsg()->getRequester();
   ...
   switch (curr_dstate) {
   case DirectoryState::EXCLUSIVE: // Cache may have done a silent upgrade to MODIFIED, so send a FLUSH (dirty data)
   ...
   case DirectoryState::MODIFIED: {
      ...
      getMemoryManager()->sendMsg(ShmemMsg::FLUSH_REQ, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
                                  requester /* requester */, directory_entry->getOwner() /* receiver */, address, NULL,
                                  0, HitWhere::UNKNOWN, shmem_req->getShmemMsg()->getPerf(),
                                  ShmemPerfModel::_SIM_THREAD);
      break;
   }
      ...
      if (sharers_list_pair.first == true) {
         ...
         for (UInt32 i = 0; i < sharers_list_pair.second.size(); i++) {
            ...
            getMemoryManager()->sendMsg(
                ShmemMsg::INV_REQ, MemComponent::TAG_DIR, MemComponent::L2_CACHE, requester /* requester */,
                sharers_list_pair.second[i] /* receiver */, address, NULL, 0, HitWhere::UNKNOWN,
                i == 0 ? shmem_req->getShmemMsg()->getPerf() : &m_dummy_shmem_perf, ShmemPerfModel::_SIM_THREAD);
         }
      }
      break;
   }
...
void DramDirectoryCntlr::processShReqFromL2Cache(ShmemReq *shmem_req, Byte *cached_data_buf)
{
   IntPtr address = shmem_req->getShmemMsg()->getAddress();
   core_id_t requester = shmem_req->getShmemMsg()->getRequester();
   ...
   case DirectoryState::EXCLUSIVE: {
      ...
      assert(cached_data_buf == NULL);
      getMemoryManager()->sendMsg(ShmemMsg::WB_REQ, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
                                  requester /* requester */, directory_entry->getOwner() /* receiver */, address, NULL,
                                  0, HitWhere::UNKNOWN, shmem_req->getShmemMsg()->getPerf(),
                                  ShmemPerfModel::_SIM_THREAD);
      break;
   ...
   case DirectoryState::SHARED: {
      if (directory_entry->hasSharer(requester)) {
         ...
         if (add_result == false) {
            ...
            getMemoryManager()->sendMsg(ShmemMsg::INV_REQ, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
                                        requester /* requester */, sharer_id /* receiver */, address, NULL, 0,
...
void DramDirectoryCntlr::retrieveDataAndSendToL2Cache(ShmemMsg::msg_t reply_msg_type, core_id_t receiver,
                                                      IntPtr address, Byte *cached_data_buf, ShmemMsg *orig_shmem_msg)
{
   DirectoryEntry *directory_entry = m_dram_directory_cache->getDirectoryEntry(address);
   ...
   if (cached_data_buf != NULL) {
      ...
      getMemoryManager()->sendMsg(reply_msg_type, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
                                  receiver /* requester */, receiver /* receiver */, address, cached_data_buf,
                                  getCacheBlockSize(),
      ...
      if (m_nuca_cache) {
         ...
         boost::tie(nuca_latency, hit_where) = m_nuca_cache->read(
             address, nuca_data_buf, getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_SIM_THREAD),
         ...
         if (hit_where != HitWhere::MISS) {
            getMemoryManager()->sendMsg(reply_msg_type, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
                                        receiver /* requester */, receiver /* receiver */, address, nuca_data_buf,
                                        getCacheBlockSize(), HitWhere::NUCA_CACHE, orig_shmem_msg->getPerf(),
                                        ShmemPerfModel::_SIM_THREAD);
      ...
      getMemoryManager()->sendMsg(PrL1PrL2DramDirectoryMSI::ShmemMsg::DRAM_READ_REQ, MemComponent::TAG_DIR,
                                  MemComponent::DRAM, receiver /* requester */, dram_node /* receiver */, address, NULL,
                                  0, HitWhere::UNKNOWN, orig_shmem_msg->getPerf(), ShmemPerfModel::_SIM_THREAD);
   }
   MYLOG("End @ %lx", address);
}

void DramDirectoryCntlr::processDRAMReply(core_id_t sender, ShmemMsg *shmem_msg)
{
   IntPtr address = shmem_msg->getAddress();
   ...
   // Keep a copy in NUCA
   sendDataToNUCA(address, shmem_req->getShmemMsg()->getRequester(), shmem_msg->getDataBuf(),
...
void DramDirectoryCntlr::processInvRepFromL2Cache(core_id_t sender, ShmemMsg *shmem_msg)
{
   IntPtr address = shmem_msg->getAddress();
...
void DramDirectoryCntlr::processUpgradeReqFromL2Cache(ShmemReq *shmem_req, Byte *cached_data_buf)
{
   ...
   case DirectoryState::MODIFIED: {
      if (sharers_list_pair.second[0] == requester) {
         ...
         getMemoryManager()->sendMsg(ShmemMsg::UPGRADE_REP, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
                                     requester /* requester */, requester /* receiver */, address, NULL, 0,
         ...
         getMemoryManager()->sendMsg(ShmemMsg::FLUSH_REQ, MemComponent::TAG_DIR, MemComponent::L2_CACHE,
                                     requester /* requester */, directory_entry->getOwner() /* receiver */, address,
                                     NULL, 0, HitWhere::UNKNOWN, shmem_msg->getPerf(), ShmemPerfModel::_SIM_THREAD);
      }
   ...
   case DirectoryState::SHARED: {
      if ((sharers_list_pair.second.size() == 1) && (sharers_list_pair.second[0] == requester)) {
         ...
         if (!requesterHasCopy) {
            MYLOG("UPGRADE_REQ: %lu sharer(s), but the requester isn't holding a copy!?",
            ...
            for (UInt32 i = 0; i < sharers_list_pair.second.size(); i++) {
               if (sharers_list_pair.second[i] != requester) {
                  ...
                  getMemoryManager()->sendMsg(msg_type, // ShmemMsg::INV_REQ,
                                              MemComponent::TAG_DIR, MemComponent::L2_CACHE, requester /* requester */,
...
void DramDirectoryCntlr::processFlushRepFromL2Cache(core_id_t sender, ShmemMsg *shmem_msg)
{
   IntPtr address = shmem_msg->getAddress();
...
void DramDirectoryCntlr::processWbRepFromL2Cache(core_id_t sender, ShmemMsg *shmem_msg)
{
   IntPtr address = shmem_msg->getAddress();
   SubsecondTime now = getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_SIM_THREAD);
   ...
   if (m_dram_directory_req_queue_list->size(address) != 0) {
      ...
      LOG_ASSERT_ERROR(shmem_req->getShmemMsg()->getMsgType() == ShmemMsg::SH_REQ, "Address(0x%x), Req(%u)", address,
...
void DramDirectoryCntlr::sendDataToNUCA(IntPtr address, core_id_t requester, Byte *data_buf, SubsecondTime now,
                                        bool count)
{
   if (m_nuca_cache) {
      bool eviction;
      ...
      m_nuca_cache->write(address, data_buf, eviction, evict_address, evict_buf,
                          getShmemPerfModel()->getElapsedTime(ShmemPerfModel::_SIM_THREAD), count);

      if (eviction) {
         // Write data to Dram
         core_id_t dram_node = m_dram_controller_home_lookup->getHome(evict_address);

         getMemoryManager()->sendMsg(PrL1PrL2DramDirectoryMSI::ShmemMsg::DRAM_WRITE_REQ, MemComponent::TAG_DIR,
                                     MemComponent::DRAM, m_core_id /* requester */, dram_node /* receiver */,
                                     evict_address, evict_buf, getCacheBlockSize(), HitWhere::UNKNOWN,
                                     &m_dummy_shmem_perf, ShmemPerfModel::_SIM_THREAD);
      }
   }
   MYLOG("End");
}

void DramDirectoryCntlr::sendDataToDram(IntPtr address, core_id_t requester, Byte *data_buf, SubsecondTime now)
{
   MYLOG("Start @ %lx", address);
   ...
   if (m_nuca_cache) {
      ...
      getMemoryManager()->sendMsg(PrL1PrL2DramDirectoryMSI::ShmemMsg::DRAM_WRITE_REQ, MemComponent::TAG_DIR,
                                  MemComponent::DRAM, requester /* requester */, dram_node /* receiver */, address,
                                  data_buf, getCacheBlockSize(), HitWhere::UNKNOWN, &m_dummy_shmem_perf,
                                  ShmemPerfModel::_SIM_THREAD);
