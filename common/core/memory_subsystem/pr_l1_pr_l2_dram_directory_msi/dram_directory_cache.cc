...
DramDirectoryCache::DramDirectoryCache(core_id_t core_id, String directory_type_str, UInt32 total_entries,
    ...
    : m_total_entries(total_entries), m_associativity(associativity), m_cache_block_size(cache_block_size),
      m_dram_directory_cache_access_time(dram_directory_cache_access_time), m_shmem_perf_model(shmem_perf_model)
{
...
DirectoryEntry *DramDirectoryCache::getDirectoryEntry(IntPtr address, bool modeled)
{
...
void DramDirectoryCache::getReplacementCandidates(IntPtr address,
                                                  std::vector<DirectoryEntry *> &replacement_candidate_list)
{
   ...
   for (UInt32 i = 0; i < m_associativity; i++) {
      replacement_candidate_list.push_back(m_directory->getDirectoryEntry(
          set_index * m_associativity + ((i + m_replacement_ptrs[set_index]) % m_associativity)));
   }
   ++m_replacement_ptrs[set_index];
}

DirectoryEntry *DramDirectoryCache::replaceDirectoryEntry(IntPtr replaced_address, IntPtr address, bool modeled)
{
   if (m_shmem_perf_model && modeled)
      getShmemPerfModel()->incrElapsedTime(m_dram_directory_cache_access_time.getLatency(),
                                           ShmemPerfModel::_SIM_THREAD);
...
void DramDirectoryCache::splitAddress(IntPtr address, IntPtr &tag, UInt32 &set_index)
{
