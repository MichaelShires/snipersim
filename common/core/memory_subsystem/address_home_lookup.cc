...
AddressHomeLookup::AddressHomeLookup(UInt32 ahl_param, std::vector<core_id_t> &core_list, UInt32 cache_block_size)
    : m_ahl_param(ahl_param), m_ahl_mask((UInt64(1) << ahl_param) - 1), m_core_list(core_list),
      m_cache_block_size(cache_block_size)
{
   ...
   LOG_ASSERT_ERROR((1 << m_ahl_param) >= (SInt32)m_cache_block_size, "2^AHL param(%u) must be >= Cache Block Size(%u)",
...
core_id_t AddressHomeLookup::getHome(IntPtr address) const
{
   ...
   LOG_ASSERT_ERROR(0 <= module_num && module_num < (SInt32)m_total_modules, "module_num(%i), total_modules(%u)",
                    module_num, m_total_modules);
