...
__attribute__((weak)) void applicationMemCopy(void *dest, const void *src, size_t n);
...
#define MYLOG(...)                                                                                                     \
   {                                                                                                                   \
      ScopedLock l(iolock);                                                                                            \
      fflush(stderr);                                                                                                  \
      fprintf(stderr, "[%8lu] %dcor %-25s@%03u: ", getPerformanceModel()->getCycleCount(ShmemPerfModel::_USER_THREAD), \
...
const char *core_state_names[] = {
    "running", "initializing", "stalled", "sleeping", "waking_up", "idle", "broken",
};
...
Core::Core(SInt32 id, SimulationContext *context)
    : m_core_id(id), m_dvfs_domain(context->getDvfsManager()->getCoreDomain(id)), m_thread(NULL), m_bbv(id),
      m_topology_info(new TopologyInfo(id)),
      m_cheetah_manager(context->getConfigFile()->getBool("core/cheetah/enabled") ? new CheetahManager(id) : NULL),
      m_dvfs_manager(context->getDvfsManager()), m_stats_manager(context->getStatsManager()),
      m_fault_injection_manager(context->getFaultinjectionManager()), m_hooks_manager(context->getHooksManager()),
      m_core_manager(context->getCoreManager()), m_config(context->getConfig()), m_cfg(context->getConfigFile()),
      m_simulator(context->getSimulator()), m_context(context),
      m_core_state(Core::IDLE), m_icache_last_block(-1), m_spin_loops(0), m_spin_instructions(0),
      m_spin_elapsed_time(SubsecondTime::Zero()), m_instructions(0), m_instructions_callback(UINT64_MAX),
      m_instructions_hpi_callback(0), m_instructions_hpi_last(0)
{
   LOG_PRINT("Core ctor for: %d", id);

   registerStatsMetric("core", id, "instructions", &m_instructions, m_stats_manager);
   registerStatsMetric("core", id, "spin_loops", &m_spin_loops, m_stats_manager);
   registerStatsMetric("core", id, "spin_instructions", &m_spin_instructions, m_stats_manager);
   registerStatsMetric("core", id, "spin_elapsed_time", &m_spin_elapsed_time, m_stats_manager);

   m_stats_manager->logTopology("hwcontext", id, id);

   m_network = new Network(this);

   m_clock_skew_minimization_client = ClockSkewMinimizationClient::create(this, m_context);

   m_shmem_perf_model = new ShmemPerfModel();

   LOG_PRINT("instantiated memory manager model");
   m_memory_manager = MemoryManagerBase::createMMU(
       m_cfg->getString("caching_protocol/type"), this, m_network, m_shmem_perf_model, m_context);
...
bool Core::countInstructions(IntPtr address, UInt32 count)
{
   ...
   if (isEnabledInstructionsCallback()) {
      if (m_instructions >= m_instructions_callback) {
         ...
         m_hooks_manager->callHooks(HookType::HOOK_INSTR_COUNT, m_core_id);
...
void Core::updateSpinCount(UInt64 instructions, SubsecondTime elapsed_time)
{
   m_spin_loops++;
...
void Core::hookPeriodicInsCheck()
{
   if (m_instructions > m_instructions_hpi_callback) {
      ...
      m_instructions_hpi_callback += m_config->getHPIInstructionsPerCore();
...
void Core::hookPeriodicInsCall()
{
   ...
   ScopedLock sl(m_core_manager->getThreadManager()->getLock());

   // Definitive, locked checked if we should do the HOOK_PERIODIC_INS callback
   if (g_instructions_hpi_global > g_instructions_hpi_global_callback) {
      m_hooks_manager->callHooks(HookType::HOOK_PERIODIC_INS, g_instructions_hpi_global);
      g_instructions_hpi_global_callback += m_config->getHPIInstructionsGlobal();
   }
}
...
MemoryResult Core::readInstructionMemory(IntPtr address, UInt32 instruction_size)
{
   LOG_PRINT("Instruction: Address(0x%x), Size(%u), Start READ", address, instruction_size);
   ...
   return initiateMemoryAccess(MemComponent::L1_ICACHE, Core::NONE, Core::READ, address & blockmask, NULL,
...
MemoryResult Core::initiateMemoryAccess(MemComponent::component_t mem_component, lock_signal_t lock_signal,
                                         mem_op_t mem_op_type, IntPtr address, Byte *data_buf, UInt32 data_size,
                                         MemModeled modeled, IntPtr eip, SubsecondTime now, bool is_fault_mask)
{
   MYLOG("access %lx+%u %c%c modeled(%s)", address, data_size, mem_op_type == Core::WRITE ? 'W' : 'R',
   ...
   if ((i++) % 1000 == 0) {
      ...
      for(int i = 0; i < m_config->getApplicationCores(); ++i)
        if (i == m_core_id)
   ...
   LOG_PRINT("Time(%s), %s - ADDR(0x%x), data_size(%u), START", itostr(initial_time).c_str(),
   ...
   for (IntPtr curr_addr_aligned = begin_addr_aligned; curr_addr_aligned <= end_addr_aligned;
        curr_addr_aligned += cache_block_size)
   {
      ...
      LOG_PRINT("Start InitiateSharedMemReq: ADDR(0x%x), offset(%u), curr_size(%u)", curr_addr_aligned, curr_offset,
                curr_size);
      ...
      HitWhere::where_t this_hit_where = getMemoryManager()->coreInitiateMemoryAccess(
          mem_component, lock_signal, mem_op_type, curr_addr_aligned, curr_offset,
          data_buf ? curr_data_buffer_head : NULL, curr_size, modeled);
      ...
      LOG_PRINT("End InitiateSharedMemReq: ADDR(0x%x), offset(%u), curr_size(%u)", curr_addr_aligned, curr_offset,
                curr_size);
   ...
   LOG_ASSERT_ERROR(final_time >= initial_time, "final_time(%s) < initial_time(%s)", itostr(final_time).c_str(),
 ...
 */
MemoryResult Core::accessMemory(lock_signal_t lock_signal, mem_op_t mem_op_type, IntPtr d_addr, char *data_buffer,
                                UInt32 data_size, MemModeled modeled, IntPtr eip, SubsecondTime now, bool is_fault_mask)
{
   // In PINTOOL mode, if the data is requested, copy it to/from real memory
   if (data_buffer && !is_fault_mask) {
      if (m_config->getSimulationMode() == Config::PINTOOL) {
         nativeMemOp(NONE, mem_op_type, d_addr, data_buffer, data_size);
      }
      else if (m_config->getSimulationMode() == Config::STANDALONE) {
         m_core_manager->getTraceManager()->accessMemory(m_core_id, lock_signal, mem_op_type, d_addr, data_buffer, data_size);
      }
   ...
   if (modeled == MEM_MODELED_NONE)
      ...
      return initiateMemoryAccess(MemComponent::L1_DCACHE, lock_signal, mem_op_type, d_addr, (Byte *)data_buffer,
                                  data_size, modeled, eip, now, is_fault_mask);
}

MemoryResult Core::nativeMemOp(lock_signal_t lock_signal, mem_op_t mem_op_type, IntPtr d_addr, char *data_buffer,
                               UInt32 data_size)
{
   if (data_size <= 0) {
      return makeMemoryResult(HitWhere::UNKNOWN, SubsecondTime::Zero());
   ...
   if (lock_signal == LOCK) {
      if (mem_op_type != READ_EX) {
         throw SimulationException("LOCK signal must be used with READ_EX operation.");
      }
   ...
   if (lock_signal == UNLOCK) {
      if (mem_op_type != WRITE) {
         throw SimulationException("UNLOCK signal must be used with WRITE operation.");
      }
...
void Core::emulateCpuid(UInt32 eax, UInt32 ecx, cpuid_result_t &res) const
{
   ...
   case 0x1: {
      ...
      cpuid(eax, ecx, res);
      res.ebx = (m_core_id << 24) | (m_config->getApplicationCores() << 16) | (res.ebx & 0xffff);
      break;
   }
   case 0xb: {
