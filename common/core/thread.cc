...
Thread::Thread(thread_id_t thread_id, app_id_t app_id)
    : m_thread_id(thread_id), m_app_id(app_id), m_core(NULL), m_rtn_tracer(NULL), m_va2pa_func(NULL), m_va2pa_arg(0)
{
...
void Thread::setCore(Core *core)
{
   ...
   if (m_core) {
      LOG_ASSERT_ERROR(core->getThread() == NULL, "Cannot move thread %d to core %d as it is already running thread %d",
...
bool Thread::reschedule(SubsecondTime &time, Core *current_core)
{
   if (m_core == NULL || current_core != m_core) {
      ...
      // Do a definitive check for m_core, unoptimized because another thread will be changing it
      while ((volatile Core *)m_core == NULL)
         time = Sim()->getThreadManager()->stallThread(getId(), STALL_UNSCHEDULED, time);
...
{
   if (Sim()->getCoreManager()->getCurrentCore() != m_core) {
      if (Sim()->getCoreManager()->getCurrentCore())
