...
class Reader
{
   ...
   typedef void (*HandleCacheOnlyFunc)(void *arg, uint8_t icount, Sift::CacheOnlyType type, uint64_t eip,
                                       uint64_t address);
   ...
   typedef void (*HandleRoutineChange)(void *arg, Sift::RoutineOpType event, uint64_t eip, uint64_t esp,
                                       uint64_t callEip);
   typedef void (*HandleRoutineAnnounce)(void *arg, uint64_t eip, const char *name, const char *imgname,
 ...
 public:
   ...
   bool AccessMemory(MemoryLockType lock_signal, MemoryOpType mem_op, uint64_t d_addr, uint8_t *data_buffer,
                     uint32_t data_size);

   void setHandleInstructionCountFunc(HandleInstructionCountFunc func, void *arg = NULL)
   {
   ...
   void setHandleRoutineFunc(HandleRoutineChange funcChange, HandleRoutineAnnounce funcAnnounce, void *arg = NULL)
   {
      assert(funcChange);
      assert(funcAnnounce);
   ...
   uint64_t getLength();
   bool getTraceHasPhysicalAddresses() const
   {
      return m_trace_has_pa;
   }
