...
class cvofstream : public vostream
{
 private:
   std::FILE *stream;

 public:
   cvofstream(const char *filename, std::ios_base::openmode mode = std::ios_base::out);
   virtual ~cvofstream();
   virtual void write(const char *s, std::streamsize n);
   virtual void flush();
   virtual bool fail();
...
class ChecksumVostream : public vostream
{
 private:
   vostream *m_output;
   uint32_t m_checksum;
 ...
 public:
   ChecksumVostream(vostream *output) : m_output(output), m_checksum(0)
   {
   }
   virtual void write(const char *s, std::streamsize n)
   {
      for (std::streamsize i = 0; i < n; ++i)
         m_checksum ^= (uint8_t)s[i];
      m_output->write(s, n);
   }
   virtual void flush()
   {
...
class ChecksumVistream : public vistream
{
 private:
   vistream *m_input;
   uint32_t m_checksum;
 ...
 public:
   ChecksumVistream(vistream *input) : m_input(input), m_checksum(0)
   {
   }
...
class cvifstream : public vistream
{
 ...
 public:
   ...
   cvifstream(std::FILE *stream) : stream(stream)
   {
   }
   virtual ~cvifstream();
   virtual void read(char *s, std::streamsize n);
