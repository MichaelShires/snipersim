...
KNOB<UINT64> KnobMPIImplicitROI(KNOB_MODE_WRITEONCE, "pintool", "sniper:roi-mpi", "0",
...
KNOB<UINT64> KnobDetailedTarget(KNOB_MODE_WRITEONCE, "pintool", "sniper:d", "0",
                                "instructions to trace in detail (default = all)");
KNOB<UINT64> KnobUseResponseFiles(
    KNOB_MODE_WRITEONCE, "pintool", "sniper:r", "0",
    "use response files (required for multithreaded applications or when emulating syscalls, default = 0)");
KNOB<UINT64> KnobEmulateSyscalls(KNOB_MODE_WRITEONCE, "pintool", "sniper:e", "0",
                                 "emulate syscalls (required for multithreaded applications, default = 0)");
KNOB<BOOL> KnobSendPhysicalAddresses(KNOB_MODE_WRITEONCE, "pintool", "sniper:pa", "0",
                                     "send logical to physical address mapping");
KNOB<UINT64> KnobFlowControl(KNOB_MODE_WRITEONCE, "pintool", "sniper:flow", "1000",
                             "number of instructions to send before syncing up");
KNOB<UINT64>
    KnobFlowControlFF(KNOB_MODE_WRITEONCE, "pintool", "sniper:flowff", "100000",
...
KNOB<BOOL> KnobRoutineTracingOutsideDetailed(KNOB_MODE_WRITEONCE, "pintool", "sniper:rtntrace_outsidedetail", "0",
                                             "routine tracing");
...
KNOB<UINT64> KnobMaxThreads(KNOB_MODE_WRITEONCE, "pintool", "sniper:maxthreads", "0",
                            "maximum number of threads (0 = default)");

KNOB<INT64> KnobPacSimEnable(KNOB_MODE_WRITEONCE, "pintool", "pacsim", "0",
                             "enable pacsim simulation; 0: disable, 1: enable");
...
KNOB<uint64_t> KnobMinimumSampledRegionSize(KNOB_MODE_WRITEONCE, "pintool", "minimum-region-size", "2000",
                                            "Sampled region size (K)");

KNOB<std::string> KnobMtngDir(KNOB_MODE_WRITEONCE, "pintool", "mtng-dir", ".",
                              "mtng data directory containing outputs from BarrierPoint and Simpoint");

KNOB<std::string> KnobMtngClusterType(KNOB_MODE_WRITEONCE, "pintool", "cluster-type", "bbv",
                                      "mtng cluster type such as bbv, ldv or sv");
...
#if defined(SDE_INIT)
KNOB<BOOL> KnobReplayer(KNOB_MODE_WRITEONCE, KNOB_FAMILY, "dummy", "0", "Replay a pinball");
#else
