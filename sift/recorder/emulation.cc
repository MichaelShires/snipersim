...
static void handleCpuid_assign(THREADID threadid, ADDRINT *gax, ADDRINT *gbx, ADDRINT *gcx, ADDRINT *gdx)
{
   ...
   // printf("After handle CPUID saved: %08x %08x %08x %08x\n", g_reply.cpuid.eax, g_reply.cpuid.ebx, g_reply.cpuid.ecx,
   // g_reply.cpuid.edx);
...
void emuKmpReapMonitor(THREADID threadIndex, CONTEXT *ctxt)
{
   ...
   void *res;
   PIN_CallApplicationFunction(ctxt, threadIndex, CALLINGSTD_DEFAULT, ptr_exit, NULL, PIN_PARG(void *), &res,
                               PIN_PARG(int), 0, PIN_PARG_END());
}

static void insCallback(INS ins, VOID *v)
{
   if (INS_IsRDTSC(ins))
      INS_InsertPredicatedCall(ins, IPOINT_AFTER, (AFUNPTR)handleRdtsc, IARG_THREAD_ID, IARG_REG_REFERENCE, REG_GAX,
                               IARG_REG_REFERENCE, REG_GDX, IARG_END);

   if (INS_Opcode(ins) == XED_ICLASS_CPUID) {
      INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)handleCpuid, IARG_THREAD_ID, IARG_REG_REFERENCE, REG_GAX,
                               IARG_REG_REFERENCE, REG_GBX, IARG_REG_REFERENCE, REG_GCX, IARG_REG_REFERENCE, REG_GDX,
                               IARG_END);
      INS_InsertPredicatedCall(ins, IPOINT_AFTER, (AFUNPTR)handleCpuid_assign, IARG_THREAD_ID, IARG_REG_REFERENCE,
                               REG_GAX, IARG_REG_REFERENCE, REG_GBX, IARG_REG_REFERENCE, REG_GCX, IARG_REG_REFERENCE,
                               REG_GDX, IARG_END);
      INS_Delete(ins);
   }
}
...
static void rtnCallback(RTN rtn, VOID *v)
{
   ...
   if (rtn_name == "sched_getcpu")
      ...
      RTN_ReplaceSignature(rtn, AFUNPTR(emuClockGettime), IARG_THREAD_ID, IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
                           IARG_FUNCARG_ENTRYPOINT_VALUE, 1, IARG_END);
   else if (rtn_name.find("gettimeofday") != std::string::npos)
      RTN_ReplaceSignature(rtn, AFUNPTR(emuGettimeofday), IARG_THREAD_ID, IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
                           IARG_FUNCARG_ENTRYPOINT_VALUE, 1, IARG_END);

   // save pointers to functions we'll want to call through PIN_CallApplicationFunction
   if (rtn_name == "exit")
      ptr_exit = RTN_Funptr(rtn);
}
