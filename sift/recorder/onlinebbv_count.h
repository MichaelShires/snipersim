#ifndef __SIFT_RECORDER_ONLINE_BBV_COUNT_H
#define __SIFT_RECORDER_ONLINE_BBV_COUNT_H
...
class OnlineBbv
{
 ...
 public:
   ...
   OnlineBbv(uint32_t tid);
   OnlineBbv(uint32_t tid, std::vector<uint64_t> &bbv_counts);
...
class Bbv_util
{
 private:
   ...
   static std::vector<std::vector<OnlineBbv>> m_BbvProfile; // store barrier_bbv.txt
 ...
 public:
   ...
   static std::vector<double> randomProjection(const std::vector<std::pair<uint64_t, uint64_t>> &standardBbv);

   static int findBbvCluster(const std::vector<std::pair<uint64_t, uint64_t>> &standardBbv);

   // getter
   static const std::vector<std::vector<std::vector<std::pair<uint64_t, uint64_t>>>> &getBbvStandardProfile()
   {
   ...
   static uint64_t getBbvDimension()
   {
      return m_bbvDim;
   }

   static uint64_t getMaxBbvId()
   {
      return m_maxBbvId;
   }

   static uint64_t getClusterSize()
   {
      return m_BbvClusterCenters.size();
   }
