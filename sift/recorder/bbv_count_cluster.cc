...
class DistanceMatrix
{
 public:
   ...
   inline bool skip_check(uint64_t cluster_id, double distance, uint64_t current_best_cluster_id)
   {
      ...
      //        std::cout << "opt:"<< cluster_id << " "<<current_best_cluster_id<< " " <<
      //        distanceMatrix[current_best_cluster_id][cluster_id]<<" " <<d1 << " " << distance << std::endl;
      if (d1 > distance) {
...
class BBVCluster
{
 ...
 public:
   ...
   std::pair<double, bool> is_same_cluster(const double *bbv_base, const double *realBbv, const int max_thread_num,
                                           double threshold)
   {
   ...
   BBVStruct *find_closed_one(double *interval_bbv, uint32_t freq, std::vector<BBVStruct *> &target_bbv_struct,
                              const int max_thread_num, double threshold)
   {
#define INIT_VALUE 10
      ...
      for (int32_t i = (int32_t)(target_bbv_struct.size()) - 1; i >= 0; i--) {
         ...
         if (min_distance != INIT_VALUE && distance_matrix.skip_check(bbv_struct.cluster_id, min_distance,
                                                                      target_bbv_struct[finally_result]->cluster_id))
         {
   ...
   std::pair<BBVStruct *, bool> get_cluster_id(double *realBbv, uint32_t freq, const int max_thread_num,
                                               bool only_search_detailed_vec = false)
   {

      BBVStruct *bbv_struct =
          find_closed_one<dvfs_enable, true>(realBbv, freq, bbv_cluster_vec_detailed, max_thread_num, threshold);
      ...
      if (bbv_struct == NULL) {
         ...
         if (only_search_detailed_vec) {
            ...
            bbv_struct =
                find_closed_one<dvfs_enable, false>(realBbv, freq, bbv_cluster_vec_ffw, max_thread_num, threshold);
            if (bbv_struct == NULL) {
   ...
   template <bool dvfs_enable>
   uint64_t add_bbv(const std::vector<UInt64> &bbv, std::vector<uint64_t> &bbv_interval_insnum,
                    const int max_thread_num, mtng::SimMode sim_mode, uint32_t freq)
   {
...
uint64_t add_bbv_intra_bbv(const std::vector<UInt64> &bbv, std::vector<uint64_t> &bbv_interval_insnum,
                           const int max_thread_num, mtng::SimMode sim_mode, uint32_t freq)
{
   if (freq == 0) {
...
// uint64_t add_bbv_inter_bbv( const std::vector<UInt64>&  bbv , std::vector<uint64_t> &bbv_interval_insnum, const int
// max_thread_num, mtng::SimMode sim_mode) {
//
...
uint64_t final_intrabarrier_cluster_id(uint32_t idx, uint32_t freq, const int max_thread_num)
{
