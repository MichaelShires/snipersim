...
uint64_t add_bbv_intra_bbv(const std::vector<UInt64> &bbv, std::vector<uint64_t> &bbv_interval_insnum,
                           int max_thread_num, mtng::SimMode sim_mode, uint32_t freq = 0);
uint64_t add_bbv_inter_bbv(const std::vector<UInt64> &bbv, std::vector<uint64_t> &bbv_interval_insnum,
                           int max_thread_num, mtng::SimMode sim_mode);
...
void final_intrabarrier_remove_uselesscluster_id(std::vector<int> &useful_cluster);
