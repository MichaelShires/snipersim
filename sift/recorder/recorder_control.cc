...
VOID call_extrae_event(const CONTEXT *ctxt, THREADID threadid, unsigned long event, unsigned long value)
{
   ...
   if (extrae_event_rtn == NULL) {
      ...
      std::cerr << "[SIFT_RECORDER:" << app_id << "] Calling extrae_event func. <" << std::dec << event
                << ">: " << value << std::endl;
   }
   ...
   PIN_CallApplicationFunction(ctxt, threadid, CALLINGSTD_DEFAULT, extrae_event_rtn, &param, PIN_PARG(void),
                               PIN_PARG(unsigned long), ROI_ON_EEVENT, PIN_PARG(unsigned long), value, PIN_PARG_END());
}
...
static void routineCallback(RTN rtn, void *v)
{
   ...
   if (KnobMPIImplicitROI.Value()) {
      ...
      if (rtn_name.find("MPI_Init") != std::string::npos &&
          rtn_name.find("MPI_Initialized") ==
              std::string::npos && // Actual name can be MPI_Init, MPI_Init_thread, PMPI_Init_thread, etc.
          aux_wrapper &&
          could_instruments)
      {
         RTN_Open(rtn);
         RTN_InsertCall(rtn, IPOINT_AFTER, AFUNPTR(handleRoutineImplicitROI), IARG_THREAD_ID, IARG_CONTEXT, IARG_BOOL,
                        true, IARG_END);
         RTN_Close(rtn);
      }
      if (rtn_name.find("MPI_Finalize") != std::string::npos && aux_wrapper && could_instruments) {
         RTN_Open(rtn);
         RTN_InsertCall(rtn, IPOINT_BEFORE, AFUNPTR(handleRoutineImplicitROI), IARG_THREAD_ID, IARG_CONTEXT, IARG_BOOL,
                        false, IARG_END);
         RTN_Close(rtn);
      }
   }
   ...
   if (in_extrae && (KnobUseROI.Value() || KnobMPIImplicitROI.Value())) {
      ...
      {
         std::cerr << "[SIFT_RECORDER:" << app_id << "] Extrae_event has been detected <" << rtn_name << ">."
                   << std::endl;
...
void openFile(THREADID threadid)
{
   ...
   if (KnobUseResponseFiles.Value() == 0) {
      ...
      if (blocksize)
         sprintf(filename, "%s.%" PRIu64 ".app%" PRId32 ".th%" PRIu64 ".sift", KnobOutputFile.Value().c_str(),
                 thread_data[threadid].blocknum, app_id, thread_data[threadid].thread_num);
      else
         sprintf(filename, "%s.app%" PRId32 ".th%" PRIu64 ".sift", KnobOutputFile.Value().c_str(), app_id,
                 thread_data[threadid].thread_num);
   }

   if (KnobVerbose.Value())
      std::cerr << "[SIFT_RECORDER:" << app_id << ":" << thread_data[threadid].thread_num << "] Output = [" << filename
                << "]" << std::endl;

   if (KnobUseResponseFiles.Value()) {
      sprintf(response_filename, "%s_response.app%" PRId32 ".th%" PRIu64 ".sift", KnobOutputFile.Value().c_str(),
              app_id, thread_data[threadid].thread_num);
      if (KnobVerbose.Value())
         std::cerr << "[SIFT_RECORDER:" << app_id << ":" << thread_data[threadid].thread_num << "] Response = ["
...
// Open the file for writing
   ...
   thread_data[threadid].output =
       new Sift::Writer(filename, getCode, KnobUseResponseFiles.Value() ? false : true, response_filename, threadid,
                        arch32, false, KnobSendPhysicalAddresses.Value());

   if (!thread_data[threadid].output->IsOpen()) {
      std::cerr << "[SIFT_RECORDER:" << app_id << ":" << thread_data[threadid].thread_num
                << "] Error: Unable to open the output file " << filename << std::endl;
      exit(1);
   }
...
{
   if (KnobVerbose.Value()) {
      std::cerr << "[SIFT_RECORDER:" << app_id << ":" << thread_data[threadid].thread_num << "] Recorded "
