...
ToolWarmup::ToolWarmup(Sift::Writer *_output, int id, bool global_sync)
    : m_output(_output), m_warmup(NULL), m_mem_count(0), m_counter(0), m_id(id), m_global_sync(global_sync),
...
{
   if (m_output == NULL) {
      std::cerr << "[TOOL_WARMUP:" << m_id << "] ERROR: Cannot writeWarmupInstructions() without a valid output target."
                << std::endl;
   }
   ...
   for (std::advance(x, start); x != maxtimevec.end(); ++x) {
      ...
      if (s_debug >= 2)
         std::cout << "[TOOL_WARMUP] @ " << time_read << "," << time_written << " 0x" << std::hex << addr << std::dec
                   << std::endl;
   ...
   if (s_debug >= 1)
      std::cout << "[TOOL_WARMUP:" << m_id << "] num_lines=" << num_lines << " start=" << start
                << " dummy=" << dummy_addresses << std::endl;
...
{
   // Check to see if the code address can be handled by the warmup tool. If not, the address range must come from the
   // native application
...
VOID PinToolWarmup::handleMemoryWarmup(THREADID threadid, ADDRINT address, BOOL is_read_not_write,
                                       ADDRINT ptr_PinToolWarmup)
{
   ...
   if (INS_IsMemoryRead(ins) || INS_IsMemoryWrite(ins)) {
      for (unsigned int i = 0; i < INS_MemoryOperandCount(ins); i++) {
         if (INS_MemoryOperandIsRead(ins, i)) {
            INS_InsertCall(ins, IPOINT_BEFORE, mem_fun, IARG_THREAD_ID, IARG_MEMORYOP_EA, i, IARG_BOOL,
                           true /*is_read_not_write*/, IARG_ADDRINT, ptw, IARG_END);
            num_addresses++;
         }
         if (INS_MemoryOperandIsWritten(ins, i)) {
            INS_InsertCall(ins, IPOINT_BEFORE, mem_fun, IARG_THREAD_ID, IARG_MEMORYOP_EA, i, IARG_BOOL,
                           false /*is_read_not_write*/, IARG_ADDRINT, ptw, IARG_END);
            num_addresses++;
         }
...
VOID PinToolWarmup::threadStart(THREADID threadIndex, CONTEXT *ctxt, INT32 flags, VOID *v)
{
   ...
   if (threadIndex >= ptw->m_num_threads) {
      std::cerr << "[PINTOOLWARMUP:" << threadIndex << "] Error, thread mapping not consistent " << threadIndex
                << " != " << ptw->m_num_threads << "\n";
      return;
   }
   ...
   // ptw->m_sift_writer[ptw->m_num_threads] = new
   // Sift::Writer((std::string("test_sift.out.")+decstr(ptw->m_num_threads)).c_str() /*filename*/,
   // PinToolWarmup::getCode, false, "" /*response_filename*/, ptw->m_num_threads /*threadid*/, false /*arch32*/, false,
   // false /*KnobSendPhysicalAddresses.Value()*/ /*!warmup*/ /*is_fifo*/);
   //     ptw->m_sift_writer[ptw->m_num_threads] = thread_data[ptw->m_num_threads].output;
   //    ptw->m_tool_warmup[ptw->m_num_threads] = new ToolWarmup(ptw->m_sift_writer[ptw->m_num_threads],
   //    ptw->m_num_threads, false /*global_sync*/);
   ptw->m_sift_writer[threadIndex] = thread_data[threadIndex].output;
   ptw->m_tool_warmup[threadIndex] =
       new ToolWarmup(ptw->m_sift_writer[threadIndex], threadIndex, false /*global_sync*/);
...
void PinToolWarmup::clearTimeStamp()
{
   for (uint64_t threadid = 0; threadid < m_num_threads; threadid++) {
      //        if (threadid >= m_num_threads)
      //        {
      //            std::cerr << "[PINTOOLWARMUP] " << __FUNCTION__ << " Invalid threadid " << threadid << " "<<
      //            m_num_threads<< "\n";
      //        //    return false;
      //        }
...
ToolWarmupGlobal::ToolWarmupGlobal() : m_sent_newthread(false), m_starttime(std::time(0))
{
   std::string arch_name = KnobArch.Value();
   if (arch_name == "gainestown") {
      CACHE_SHARED_BY_CORES = 4;
...
bool ToolWarmupGlobal::writeWarmupInstructionsOldOrig()
{
   ...
   for (auto i = time2addrthread.begin(); i != time2addrthread.end(); ++i) {
      ...
      if (s_debug >= 2)
         std::cout << "[TOOL_WARMUP:" << thread << "] @ " << i->m_time << " 0x" << std::hex << addr << std::dec
                   << std::endl;
...
bool ToolWarmupGlobal::writeWarmupInstructions()
{
   ...
   size_t num_caches = ((num_threads + (CACHE_SHARED_BY_CORES - 1)) /
                        CACHE_SHARED_BY_CORES); // Round, as we need another cache for uneven sharers
   ...
   if (s_debug >= 1) {
      std::cerr << "[TOOL_WARMUP_GLOBAL] nthreads: " << num_threads << " num_caches: " << num_caches
                << " num_total_sets: " << num_total_sets << " num_total_assoc: " << num_total_assoc << std::endl;
   }
   ...
   for (auto t = m_warmup_list.begin(); t != m_warmup_list.end(); ++t, ++thread) {
      ...
      for (auto i = addr2time->begin(); i != addr2time->end(); ++i) {
         ...
         if (time_read)
            addLruThreadRead(set_time3addrthread[set_index], set_addrtoindex[set_index], addr, time_read, thread,
                             /*is_icache*/ false);
         if (time_written)
      ...
      for (auto i = insn2time->begin(); i != insn2time->end(); ++i) {
         ...
         if (s_debug)
            assert(set_index < num_total_sets);
         addLruThreadRead(set_time3addrthread[set_index], set_addrtoindex[set_index], addr, time_icache, thread,
                          /*is_icache*/ true);
      }
   ...
   for (auto setvi = set_time3addrthread.begin(); setvi != set_time3addrthread.end(); ++setvi) {
      ...
      for (std::advance(x, start); x != setv.end(); ++x) {
         ...
         // if (s_debug >= 2)
         //    std::cout << "[TOOL_WARMUP_GLOBAL] @ " << time_read << "," << time_written << " 0x" << std::hex << addr
         //    << std::dec << std::endl;
      }
   }
   ...
   for (auto i = time2addrthread.begin(); i != time2addrthread.end(); ++i) {
      ...
      uint64_t thread = i->m_thread;
      if (s_debug >= 2)
         std::cout << "[TOOL_WARMUP_GLOBAL:" << thread << "] @ " << i->m_time << " 0x" << std::hex << addr << std::dec
                   << std::endl;
      for (unsigned int m = 0; m < m_warmup_list.size(); m++) {
         if (m == thread)
            m_warmup_list[m]->_memoryAccess(addr);
