...
bool Sift::Warmup::getCode(uint8_t *dst, const uint8_t *src, uint32_t size)
{
   ...
   if ((code_start_u < codePage_u) || (code_start_u > (codePage_u + page_size)) ||
       ((code_start_u + size) < codePage_u) || ((code_start_u + size) > (codePage_u + page_size)))
   {
      return false;
   }
...
Sift::Warmup::Warmup(const char *filename, int id, Sift::Writer *_output)
{
   ...
   if (_output) {
      ...
      // Writer(const char *filename, GetCodeFunc getCodeFunc, bool useCompression = false, const char
      // *response_filename = "", uint32_t id = 0, bool arch32 = false, bool requires_icache_per_insn = false, bool
      // send_va2pa_mapping = false, GetCodeFunc2 getCodeFunc2 = NULL, void *GetCodeFunc2Data = NULL);
      output = new Sift::Writer(filename, Sift::Warmup::_getCode, false /*compression*/, "", id, false /*arch32*/,
                                false /*icache-per-insn*/, false /*physical addresses?*/);
      m_output_owner = true;
   }
}
...
void Sift::Warmup::MemoryAccess(uint64_t dataaddr, bool isread)
{
   ...
   while (is_branch) {
      ...
      output->Instruction(insnaddr, size, num_addr, addresses, is_branch, taken, false /*is_predicate*/,
                          true /*executing*/);
   }
}
