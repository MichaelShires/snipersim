...
static EXCEPT_HANDLING_RESULT pinboost_exceptionhandler(THREADID threadid, EXCEPTION_INFO *pExceptInfo,
                                                        PHYSICAL_CONTEXT *pPhysCtxt, VOID *v)
{
   ...
   if (!in_handler) {
      ...
      if (!success)
         std::cerr << "[" << pinboost_name << "] Internal exception:" << PIN_ExceptionToString(pExceptInfo)
                   << std::endl;
...
bool pinboost_backtrace(EXCEPTION_INFO *pExceptInfo, PHYSICAL_CONTEXT *pPhysCtxt)
{
   ...
   //                                               (void*)PIN_GetPhysicalContextReg(pPhysCtxt,
...
{
   std::cerr << "[" << pinboost_name << "] " << __file << ":" << __line << ": " << __function << ": Assertion `"
