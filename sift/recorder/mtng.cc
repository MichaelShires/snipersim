...
{
   std::string print[] = {"PointUndef",   "MultiThread",        "SingleThread",  "IntraBarrier",
                          "UnknownBlock", "UnknownBlockRecord", "HardWareEvent", "EndPgm"};
   out << print[(int32_t)record_type];
   return out;
}
std::ostream &operator<<(std::ostream &out, const SimMode &record_type)
{
   std::string print[] = {"Detailed", "FastForward", "Warmup"};
   out << print[(int32_t)record_type];
   return out;
}
...
class Mtng
{
 ...
 public:
   ...
   template <RecordType record_type> void changeMode(ADDRINT pc_addr)
   {
      Record record;
      ...
      if (!has_key(pc_addr)) {
         ...
         std::cout << "\033[1;32m[BBV   ] BBV simulated, start Sift::ModeIcount\033[0m" << std::endl;
...
VOID traceCallbackForBarrierPoint(TRACE trace, void *v)
{
   ...
   if (RTN_Valid(rtn)) {
      for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl)) {
         ...
         if (INS_IsCall(tail)) {
            ...
            INS_InsertPredicatedCall(tail, IPOINT_BEFORE, (AFUNPTR)recordCall, IARG_THREAD_ID, IARG_INST_PTR,
   ...
   if (RTN_Valid(rtn) && RTN_Address(rtn) == TRACE_Address(trace)) {
      ...
      if (((rtn_name.find("GOMP_parallel_start") != std::string::npos) ||
           (rtn_name.find("GOMP_parallel") != std::string::npos)) &&
          !((rtn_name.find("@plt") != std::string::npos) || (rtn_name.find("GOMP_parallel_end") != std::string::npos)))
      {
         ...
         //      if( rtn_name == OMP_END )
         if (((rtn_name.find("GOMP_parallel_end") != std::string::npos)) &&
             !((rtn_name.find("@plt") != std::string::npos)))
         {
            INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)processOmpEnd, IARG_THREAD_ID, IARG_INST_PTR,
                                     IARG_ADDRINT, v, IARG_END);
         }
   }
}
