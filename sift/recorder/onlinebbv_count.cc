...
OnlineBbv::OnlineBbv(uint32_t thread_id_para)
...
OnlineBbv::OnlineBbv(uint32_t thread_id_para, std::vector<uint64_t> &bbv_counts)
{
...
void OnlineBbv::count(uint64_t address, uint64_t count)
{
   *m_instrs += count;
...
std::vector<std::vector<OnlineBbv>> Bbv_util::m_BbvProfile;

// return 0 on success, -1 on error
// TODO inefficient due to substr operation, refactor by following loadBbvFv()
int Bbv_util::loadBbvProfile(std::string filePath)
{
   ...
   {
       std::cout << "[OnlineBbv  ] Error opening file: " << filePath << std::endl;
       return -1;
   }
   */
   ...
   // m_BbvProfile.push_back(std::vector<OnlineBbv>());
   ...
   while (getline(fin, next, ' ')) {
      ...
      if (next[0] == 'T') {
         ...
         if (!first_pass) {
            m_BbvProfile[b].push_back(OnlineBbv(last_thread_num, bbv_counts));
      ...
      {
         if (bbv_num != 0) {
            std::cout << "[OnlineBbv  ] Error when parsing file: " << filePath << std::endl;
            return -1;
         }
         m_BbvProfile[b].push_back(OnlineBbv(thread_num, bbv_counts));
         // initialise new bbv_counts for new thread
         bbv_counts = std::vector<uint64_t>(NUM_BBV, 0);
      }
...
// return 0 on success, -1 on error
int Bbv_util::validateBbvProfile(std::string filePath)
{
...
int Bbv_util::loadRandomProjectionMatrix(std::string filePath)
{
...
int Bbv_util::loadBbvStandardProfile(std::string filePath)
{
   using namespace std;
   ...
   while (getline(is, line, '\n')) {
      ...
      while (getline(ss, next, ' ')) {
         ...
         uint64_t threadid = strtoull(next.substr(delim1 + 1, delim2 - delim1 - 1).c_str(), 0, 10);
...
int Bbv_util::loadBbvClusterCenters(std::string filePath)
{
   std::ifstream is(filePath.c_str());
   if (!is.is_open()) {
      std::cerr << "[BBV ERROR] File: " << filePath << " does not exist! \n";
...
int Bbv_util::loadBbvSimpoints(std::string filePath)
{
   std::ifstream is(filePath.c_str());
   if (!is.is_open()) {
      std::cerr << "[BBV ERROR] File: " << filePath << " does not exist! \n";
...
// "weight" of OnlineBbv. Does this matter bbv is treated as rows, wich is of dim (1 x
...
std::vector<double> Bbv_util::randomProjection(const std::vector<std::pair<uint64_t, uint64_t>> &standardBbv)
{
   ...
   for (auto &bbpair : standardBbv) {
      ...
      for (unsigned int projCol = 0; projCol < NUM_BBV; projCol++) {
         ...
         bbv_dim_reduced[projCol] += bbcount * m_randomProjectionMatrix[dim][projCol]; // * m_weights[dim];
      }
   }
   /*
...
int Bbv_util::findBbvCluster(const std::vector<std::pair<uint64_t, uint64_t>> &standardBbv)
{
...
int Bbv_util::loadBbvLabels(std::string filePath)
{
   std::ifstream is(filePath.c_str());
   if (!is.is_open()) {
      std::cerr << "[BBV ERROR] File: " << filePath << " does not exist! \n";
