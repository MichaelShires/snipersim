...
struct TimeAddr
{
   ...
   TimeAddr(uint64_t _time, uint64_t _addr) : m_time(_time), m_addr(_addr)
   {
   }
...
struct TimeAddrThread
{
   ...
   TimeAddrThread(uint64_t _time, uint64_t _addr, uint64_t _thread) : m_time(_time), m_addr(_addr), m_thread(_thread)
   {
   }
...
struct Time2Addr
{
   ...
   Time2Addr(uint64_t _time0, uint64_t _time1, uint64_t _addr) : m_time{_time0, _time1}, m_addr(_addr)
   {
   }
...
struct Time3Addr
{
   ...
   Time3Addr(uint64_t _time0, uint64_t _time1, uint64_t _time2, uint64_t _addr)
       : m_time{_time0, _time1, _time2}, m_addr(_addr)
   {
   }

   bool operator<(const Time3Addr &at) const
   {
      return (std::max(std::max(m_time[0], m_time[1]), m_time[2]) <
              std::max(std::max(at.m_time[0], at.m_time[1]), at.m_time[2]));
   }
};
...
struct Time3AddrThread
{
   ...
   Time3AddrThread(uint64_t _time0, uint64_t _time1, uint64_t _time2, uint64_t _addr, uint64_t _thread)
       : m_time{_time0, _time1, _time2}, m_addr(_addr), m_thread(_thread)
   {
   }
...
struct MTR
{
   MTR() : read_times{0}, address(0), write_thread(0), write_time(0), max_time(0), is_icache{0}
   {
   }
...
inline void addLruThreadRead(std::vector<MTR> &set_mtr, std::unordered_map<uint64_t, uint64_t> &set_addrtoindex,
...
inline void addLruThreadWrite(std::vector<MTR> &set_mtr, std::unordered_map<uint64_t, uint64_t> &set_addrtoindex,
...
class ToolWarmup
{
 public:
   ...
   void initSiftWarmup();
   const AddrTime2Map *getAddr2Time() const
   {
      return &m_addr2time;
   }
   const AddrTimeMap *getInsn2Time() const
   {
      return &m_insn2time;
   }
   void _memoryAccess(uint64_t addr)
   {
      m_warmup->MemoryAccess(addr);
   } // For direct memory access. Only for use by ToolWarmupGlobal
   void _getLock()
   {
   ...
   bool _valid()
   {
      return (m_output != NULL) && (m_warmup != NULL);
   }
...
class PinToolWarmup
{
 ...
 public:
   ...
   static VOID handleMemoryWarmup(THREADID threadid, ADDRINT address, BOOL is_read_not_write,
...
class ToolWarmupGlobal
{
 public:
   ...
   void registerWarmup(ToolWarmup *tw)
   {
      m_warmup_list.push_back(tw);
   }
