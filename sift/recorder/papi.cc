...
ADDRINT emuPAPI_event_code_to_name(int EventCode, char *EventName)
{
   ...
   case PAPI_TOT_INS_CODE:
      strcpy(EventName, "emuPAPI_TOT_INS");
      break;
   case PAPI_TOT_CYC_CODE:
      strcpy(EventName, "emuPAPI_TOT_CYC");
      break;
   case PAPI_L1_DCM_CODE:
      strcpy(EventName, "emuPAPI_L1_DCM");
      break;
   case PAPI_L2_DCM_CODE:
      strcpy(EventName, "emuPAPI_L2_DCM");
      break;
   case PAPI_L3_TCM_CODE:
      strcpy(EventName, "emuPAPI_L3_TCM");
      break;
   case PAPI_BR_MSP_CODE:
      strcpy(EventName, "emuPAPI_BR_MSP");
      break;
   default:
      strcpy(EventName, "emuNOT_COUNTER");
      break;
   }
...
ADDRINT emuPAPI_event_name_to_code(char *EventName, int *EventCode)
{
   ...
   if (!strcmp(EventName, "PAPI_TOT_INS"))
      *EventCode = PAPI_TOT_INS_CODE;
   else if (!strcmp(EventName, "PAPI_TOT_CYC"))
      *EventCode = PAPI_TOT_CYC_CODE;
   else if (!strcmp(EventName, "PAPI_L1_DCM"))
      *EventCode = PAPI_L1_DCM_CODE;
   else if (!strcmp(EventName, "PAPI_L2_DCM"))
      *EventCode = PAPI_L2_DCM_CODE;
   else if (!strcmp(EventName, "PAPI_L3_TCM"))
      *EventCode = PAPI_L3_TCM_CODE;
   else if (!strcmp(EventName, "PAPI_BR_MSP"))
      *EventCode = PAPI_BR_MSP_CODE;
      ...
      for (RTN rtn = SEC_RtnHead(s); RTN_Valid(rtn); rtn = RTN_Next(rtn)) {
         ...
         if (routine_name.find("PAPI_") != std::string::npos) {
            if (routine_name.find("PAPI_start") != std::string::npos)
               ...
               RTN_ReplaceSignature(rtn, AFUNPTR(emuPAPI_read), IARG_THREAD_ID, IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
                                    IARG_FUNCARG_ENTRYPOINT_VALUE, 1, IARG_END);
            else if (routine_name.find("PAPI_event_name_to_code") != std::string::npos)
               RTN_ReplaceSignature(rtn, AFUNPTR(emuPAPI_event_name_to_code), IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
                                    IARG_FUNCARG_ENTRYPOINT_VALUE, 1, IARG_END);
            else if (routine_name.find("PAPI_event_code_to_name") != std::string::npos)
               RTN_ReplaceSignature(rtn, AFUNPTR(emuPAPI_event_code_to_name), IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
                                    IARG_FUNCARG_ENTRYPOINT_VALUE, 1, IARG_END);
            else if (routine_name.find("PAPI_library_init") != std::string::npos)
               RTN_ReplaceSignature(rtn, AFUNPTR(emuPAPI_library_init), IARG_THREAD_ID, IARG_FUNCARG_ENTRYPOINT_VALUE,
                                    0, IARG_END);
            else if (routine_name.find("PAPI_get_event_info") != std::string::npos)
               RTN_ReplaceSignature(rtn, AFUNPTR(emuPAPI_get_event_info), IARG_FUNCARG_ENTRYPOINT_VALUE, 0,
                                    IARG_FUNCARG_ENTRYPOINT_VALUE, 1, IARG_END);
            else
