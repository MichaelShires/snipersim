...
VOID countInsns(THREADID threadid, INT32 count)
{
   ...
   if (thread_data[threadid].icount >= fast_forward_target && !in_roi && !KnobUseROI.Value() &&
       !KnobMPIImplicitROI.Value())
   {
      if (KnobVerbose.Value())
         std::cerr << "[SIFT_RECORDER:" << app_id << ":" << thread_data[threadid].thread_num
                   << "] Changing to detailed after " << thread_data[threadid].icount << " instructions" << std::endl;
...
VOID sendInstruction(THREADID threadid, ADDRINT addr, UINT32 size, UINT32 num_addresses, BOOL is_branch, BOOL taken,
                     BOOL is_predicate, BOOL executing, BOOL isbefore, BOOL ispause)
{
   ...
   if (isbefore) {
      for (uint8_t i = 0; i < num_addresses; ++i) {
         // If the instruction hasn't executed yet, access the address to ensure a page fault if the mapping wasn't set
         // up yet
         static char dummy = 0;
   ...
   thread_data[threadid].output->Instruction(addr, size, num_addresses, thread_data[threadid].dyn_addresses, is_branch,
                                             taken, is_predicate, executing);
   thread_data[threadid].num_dyn_addresses = 0;

   if (KnobUseResponseFiles.Value() && KnobFlowControl.Value() &&
       (thread_data[threadid].icount > thread_data[threadid].flowcontrol_target || ispause))
   {
...
VOID sendCacheOnly(THREADID threadid, UINT32 icount, UINT32 type, ADDRINT eip, ADDRINT arg)
{
   ...
   thread_data[threadid].output->CacheOnly(thread_data[threadid].icount_cacheonly_pending, Sift::CacheOnlyType(type),
                                           eip, address);
...
UINT32 addMemoryModeling(INS ins)
{
   ...
   if (INS_IsMemoryRead(ins) || INS_IsMemoryWrite(ins)) {
      ...
      for (unsigned int i = 0; i < max_op_count; i++) {
         INS_InsertCall(ins, IPOINT_BEFORE, AFUNPTR(handleMemory), IARG_THREAD_ID, IARG_MEMORYOP_EA, i, IARG_END);
         num_addresses++;
      }
   }
   if (INS_MemoryOperandCount(ins) > Sift::MAX_DYNAMIC_ADDRESSES) {
      std::cerr << "[SIFT_RECORDER] Unable to report all dynamic addresses (" << Sift::MAX_DYNAMIC_ADDRESSES << "/"
                << INS_MemoryOperandCount(ins) << ") for instruction 0x" << std::hex << INS_Address(ins) << std::dec
                << "\n";
   }
...
VOID insertCall(INS ins, IPOINT ipoint, UINT32 num_addresses, BOOL is_branch, BOOL taken)
{
   INS_InsertCall(ins, ipoint, AFUNPTR(sendInstruction), IARG_THREAD_ID, IARG_ADDRINT, INS_Address(ins), IARG_UINT32,
                  UINT32(INS_Size(ins)), IARG_UINT32, num_addresses, IARG_BOOL, is_branch, IARG_BOOL, taken, IARG_BOOL,
                  INS_IsPredicated(ins), IARG_EXECUTING, IARG_BOOL, ipoint == IPOINT_BEFORE, IARG_BOOL,
...
static VOID traceCallback(TRACE trace, void *v)
{
   ...
   if (extrae_image.linked) {
      ...
      if (trace_address >= extrae_image.top_addr && trace_address < extrae_image.bottom_addr) {
         return;
      }
   }
   ...
   for (BBL bbl = bbl_head; BBL_Valid(bbl); bbl = BBL_Next(bbl)) {
      for (INS ins = BBL_InsHead(bbl);; ins = INS_Next(ins)) {
         ...
         if (INS_IsXchg(ins) && INS_OperandReg(ins, 0) == REG_BX && INS_OperandReg(ins, 1) == REG_BX) {
            INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)handleMagic, IARG_RETURN_REGS, REG_GAX,
                                     IARG_THREAD_ID, IARG_CONTEXT, IARG_REG_VALUE, REG_GAX,
#ifdef TARGET_IA32
         ...
         if (INS_IsSyscall(ins)) {
            INS_InsertPredicatedCall(ins, IPOINT_BEFORE, AFUNPTR(emulateSyscallFunc), IARG_THREAD_ID,
      ...
      if (!any_thread_in_detail) {
         BBL_InsertCall(bbl, IPOINT_ANYWHERE, (AFUNPTR)countInsns, IARG_THREAD_ID, IARG_UINT32, BBL_NumIns(bbl),
                        IARG_END);
      }
         ...
         for (INS ins = BBL_InsHead(bbl);; ins = INS_Next(ins)) {
            ...
            if (is_branch && INS_IsValidForIpointTakenBranch(ins) && INS_IsValidForIpointAfter(ins)) {
               ...
               insertCall(ins, INS_IsValidForIpointAfter(ins) ? IPOINT_AFTER : IPOINT_BEFORE, num_addresses,
                          false /* is_branch */, false /* taken */);
            }
         ...
         for (INS ins = BBL_InsHead(bbl);; ins = INS_Next(ins)) {
            ...
            if (INS_IsMemoryRead(ins) || INS_IsMemoryWrite(ins)) {
               ...
               for (unsigned int idx = 0; idx < INS_MemoryOperandCount(ins); ++idx) {
                  if (INS_MemoryOperandIsRead(ins, idx)) {
                     INS_InsertCall(ins, ipoint, AFUNPTR(sendCacheOnly), IARG_THREAD_ID, IARG_UINT32, inscount,
                                    IARG_UINT32, UINT32(Sift::CacheOnlyMemRead), IARG_ADDRINT, INS_Address(ins),
                                    IARG_UINT32, UINT32(idx), IARG_END);
                     inscount = 0;
                  }
                  if (INS_MemoryOperandIsWritten(ins, idx)) {
                     INS_InsertCall(ins, ipoint, AFUNPTR(sendCacheOnly), IARG_THREAD_ID, IARG_UINT32, inscount,
                                    IARG_UINT32, UINT32(Sift::CacheOnlyMemWrite), IARG_ADDRINT, INS_Address(ins),
                                    IARG_UINT32, UINT32(idx), IARG_END);
                     inscount = 0;
                  }
               }
               INS_InsertCall(ins, ipoint, AFUNPTR(cacheOnlyConsumeAddresses), IARG_THREAD_ID, IARG_END);
            }
            if (INS_IsBranch(ins) && INS_HasFallThrough(ins)) {
               INS_InsertCall(ins, IPOINT_TAKEN_BRANCH, AFUNPTR(sendCacheOnly), IARG_THREAD_ID, IARG_UINT32, inscount,
                              IARG_UINT32, UINT32(Sift::CacheOnlyBranchTaken), IARG_ADDRINT, INS_Address(ins),
                              IARG_BRANCH_TARGET_ADDR, IARG_END);
               INS_InsertCall(ins, IPOINT_AFTER, AFUNPTR(sendCacheOnly), IARG_THREAD_ID, IARG_UINT32, inscount,
                              IARG_UINT32, Sift::CacheOnlyBranchNotTaken, IARG_ADDRINT, INS_Address(ins),
            ...
            if (ins == BBL_InsTail(bbl)) {
               if (inscount)
                  INS_InsertCall(ins, IPOINT_BEFORE, AFUNPTR(cacheOnlyUpdateInsCount), IARG_THREAD_ID, IARG_UINT32,
                                 inscount, IARG_END);
               break;
            }
         }
