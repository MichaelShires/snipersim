...
static void announceRoutine(INS ins)
{
   ...
   thread_data[PIN_ThreadId()].output->RoutineAnnounce(
       eip, RTN_Valid(rtn) ? RTN_Name(rtn).c_str() : "??", IMG_Valid(img) ? IMG_Name(img).c_str() : "??",
...
static void routineCallback(RTN rtn, VOID *v)
{
   ...
   RTN_InsertCall(rtn, IPOINT_BEFORE, AFUNPTR(routineEnter), IARG_THREAD_ID, IARG_ADDRINT, RTN_Address(rtn),
                  IARG_REG_VALUE, REG_STACK_PTR, IARG_END);
   RTN_InsertCall(rtn, IPOINT_AFTER, AFUNPTR(routineExit), IARG_THREAD_ID, IARG_ADDRINT, RTN_Address(rtn),
                  IARG_REG_VALUE, REG_STACK_PTR, IARG_END);
...
static void traceCallback(TRACE trace, VOID *v)
{
   ...
   if (RTN_Valid(rtn)) {
      ...
      TRACE_InsertCall(trace, IPOINT_BEFORE, AFUNPTR(routineAssert), IARG_THREAD_ID, IARG_ADDRINT, RTN_Address(rtn),
                       IARG_REG_VALUE, REG_STACK_PTR, IARG_END);
   }
   else {
      ...
      TRACE_InsertCall(trace, IPOINT_BEFORE, AFUNPTR(routineAssert), IARG_THREAD_ID, IARG_ADDRINT, 0, IARG_REG_VALUE,
