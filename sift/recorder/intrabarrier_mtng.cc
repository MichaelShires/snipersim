...
{
//**common mtng
VOID sendInstruction(THREADID threadid, ADDRINT addr, UINT32 size, BOOL is_branch, BOOL taken, BOOL is_predicate,
                     BOOL executing, BOOL isbefore, BOOL ispause)
{
...
{
   INS_InsertCall(ins, ipoint, AFUNPTR(sendInstruction), IARG_THREAD_ID, IARG_ADDRINT, INS_Address(ins), IARG_UINT32,
                  UINT32(INS_Size(ins)), IARG_BOOL, is_branch, IARG_BOOL, taken, IARG_BOOL, INS_IsPredicated(ins),
                  IARG_EXECUTING, IARG_BOOL, ipoint == IPOINT_BEFORE, IARG_BOOL, INS_Opcode(ins) == XED_ICLASS_PAUSE,
                  IARG_END);
}
      ...
      for (BBL bbl = bbl_head; BBL_Valid(bbl); bbl = BBL_Next(bbl))
         for (INS ins = BBL_InsHead(bbl);; ins = INS_Next(ins)) {
            ...
            if (is_branch && INS_IsValidForIpointTakenBranch(ins) && INS_IsValidForIpointAfter(ins)) {
               ...
               // Whenever possible, use IPOINT_AFTER as this allows us to process addresses after the application has
               // used them. This ensures that their logical to physical mapping has been set up.
               insertCall(ins, INS_IsValidForIpointAfter(ins) ? IPOINT_AFTER : IPOINT_BEFORE, false /* is_branch */,
                          false /* taken */);
            }
...
template <bool dvfs_enable> class Feature
{
 public:
   ...
   Feature(uint64_t fs_, const std::vector<uint64_t> &insnums, mtng::RecordType record_type_in, uint64_t l2_count_,
           uint64_t l2_miss_count_)
       : fs(fs_), record_type(record_type_in), solved(true), l2_count(l2_count_),
         l2_miss_count(l2_miss_count_) // use for detailed mode
   {
   ...
   void extrapolate(const Feature *last_feature, const Feature *next_feature,
                    DataLevel feature_level_in = DataLevel::FromLocal)
   {
      solved = true;
   ...
   Feature(const std::vector<uint64_t> &insnums, mtng::RecordType record_type_in)
       : record_type(record_type_in), solved(false)
   {
      max_ins_num = 0;
      for (uint32_t i = 0; i < MAX_NUM_THREADS_DEFAULT; i++) {
         ins_nums[i] = insnums[i];
...
{
   out << "fs : " << feature.fs / 1e15 << " pc: " << feature.pc << " insnum: " << feature.max_ins_num
       << "clusterid: " << feature.cluster_id << " solved " << feature.solved;
   return out;
}
...
template <bool dvfs_enable> class Predictor
{
   ...
   std::unordered_map<FeatureKey<dvfs_enable>, Feature<dvfs_enable>>
       clusterid2feature;                                              // use history to predict cluster id
 ...
 public:
   //            Predictor()= delete;
   //
   Predictor(std::vector<uint64_t> &cluster_id_queue_para, mtng::SimMode &mtng_current_mode_para)
       : cluster_id_queue(cluster_id_queue_para), mtng_current_mode(mtng_current_mode_para) {};
...
#define MAX_HISTORY_QUEUE (16)
   void onlyUpdateClusterFeature(uint64_t cluster_id, const Feature<dvfs_enable> &feature)
   { // give up if exiting cluster id
   ...
   PredictResultType updateCurPredictor(uint64_t cluster_id, Feature<dvfs_enable> &feature)
   {
      ...
      if (!if_predict_right(cluster_id)) {
         //                    LOG2(INFO) << "predict : " << last_predicted_cluster_id << " actual cluster id : " <<
         //                    cluster_id << "\n";
...
template <bool dvfs_enable> class Mtng : public VirtualMtng
{
...
#define queue_size 1
 ...
 public:
   ...
   Mtng()
       : threshold(KnobSampledRegionSize.Value() * 1000),
         minimum_threshold(KnobMinimumSampledRegionSize.Value() * 1000), current_barrier(0)
   {
...
#define STABLE_THRESHOLD 2
   ...
   template <mtng::RecordType record_type> mtng::SimMode processRegion(ADDRINT pcaddr)
   {
      //            Record record;
   ...
   bool trySolveFeatureByLocalInfo(Feature<dvfs_enable> &feature, DataLevel datalevel = DataLevel::FromLocal)
   {
      ...
      if (predictor_it == hash2predictor.end())
         return false;
      auto &predictor = predictor_it->second;
   ...
   bool trySolveFeatureByClosedFeature(Feature<dvfs_enable> &feature, Feature<dvfs_enable> *last_feature,
                                       Feature<dvfs_enable> *next_feature)
   {
...
VOID traceCallbackForBarrierPoint(TRACE trace, void *v)
{
...

   {
      ...
      for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl)) {
         ...
         INS_InsertPredicatedCall(tail, IPOINT_BEFORE, (AFUNPTR)recordInsCount, IARG_THREAD_ID, IARG_UINT32,
                                  BBL_NumIns(bbl), IARG_INST_PTR, IARG_ADDRINT, v, IARG_END);
      }
      {
         ...
         if (INS_IsDirectControlFlow(tail) && INS_HasFallThrough(tail)) {
            ...
            if (target < current_pc) {
               INS_InsertPredicatedCall(tail, IPOINT_BEFORE, AFUNPTR(sendIntraBarrierInstruction), IARG_THREAD_ID,
                                        IARG_INST_PTR, IARG_ADDRINT, v, IARG_END);
            }
         }
      }
      // comments: end
...
#define OMP_FN_BEGIN "_omp_fn"
      if ((rtn_name.find(OMP_FN_BEGIN) != std::string::npos)) {
         INS_InsertPredicatedCall(ins, IPOINT_BEFORE, (AFUNPTR)processOmpBegin, IARG_THREAD_ID, IARG_INST_PTR,
                                  IARG_ADDRINT, v, IARG_END);
      }
   }
}

/// this function is for intel compiler to detect barrier boundary
VOID iccRoutineStartCallback(RTN rtn, void *v)
{
   RTN_Open(rtn);
   if (RTN_Valid(rtn)) {
      ...
      if (rtn_name == "__kmp_invoke_microtask") {
         ...
         for (INS tail = RTN_InsHead(rtn); INS_Valid(tail); tail = INS_Next(tail)) {
            ...
            if (INS_IsCall(tail)) {
               INS_InsertPredicatedCall(tail, IPOINT_BEFORE, (AFUNPTR)processOmpBegin, IARG_THREAD_ID, IARG_INST_PTR,
                                        IARG_ADDRINT, v, IARG_END);
            }
         }
      }
...
VOID processExit(THREADID threadid, ADDRINT pcaddr, void *v)
{
...
static void routineCallback(RTN rtn, VOID *v)
{
   ...
   if (RTN_Valid(rtn)) {
      ...
      if (rtn_name == "_exit" || rtn_name == "_Exit") {
         RTN_InsertCall(rtn, IPOINT_BEFORE, (AFUNPTR)processExit, IARG_THREAD_ID, IARG_INST_PTR, IARG_ADDRINT, v,
                        IARG_END);
      }
   }
...
template <bool dvfs_enable2> void activate()
{
