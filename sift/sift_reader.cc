...
Sift::Reader::Reader(const char *filename, const char *response_filename, uint32_t id)
    : input(NULL), response(NULL), handleInstructionCountFunc(NULL), handleInstructionCountArg(NULL),
      handleCacheOnlyFunc(NULL), handleCacheOnlyArg(NULL), handleOutputFunc(NULL), handleOutputArg(NULL),
      handleSyscallFunc(NULL), handleSyscallArg(NULL), handleNewThreadFunc(NULL), handleNewThreadArg(NULL),
      handleForkFunc(NULL), handleForkArg(NULL), handleJoinFunc(NULL), handleJoinArg(NULL), handleMagicFunc(NULL),
      handleMagicArg(NULL), handleEmuFunc(NULL), handleEmuArg(NULL), handleRoutineChangeFunc(NULL),
      handleRoutineAnnounceFunc(NULL), handleRoutineArg(NULL), filesize(0), last_address(0), icache(), m_id(id),
      m_trace_has_pa(false), m_seen_end(false), m_last_sinst(NULL), m_isa(0)
{
...
bool Sift::Reader::initStream()
{
   ...
   input = new ChecksumVistream(input);
...
bool Sift::Reader::Read(Instruction &inst)
{
   ...
   while (!m_seen_end) {
      ...
      if (byte == 0) {
         ...
         case RecOtherChecksum: {
            assert(rec.Other.size == sizeof(uint32_t));
            uint32_t expected_checksum;
            uint32_t current_checksum = static_cast<ChecksumVistream *>(input)->getChecksum();
            // XOR back the header bytes to get the checksum of everything before this record
            uint8_t *p = reinterpret_cast<uint8_t *>(&rec.Other);
            for (size_t i = 0; i < sizeof(rec.Other); ++i)
               current_checksum ^= p[i];

            input->read(reinterpret_cast<char *>(&expected_checksum), sizeof(uint32_t));
            if (current_checksum != expected_checksum) {
               std::cerr << "[SIFT:" << m_id << "] Error: Trace checksum mismatch (expected 0x" << std::hex
                         << expected_checksum << ", got 0x" << current_checksum << std::dec << "). Trace is likely corrupted.\n";
            }
            else {
         #if VERBOSE > 0
               std::cerr << "[SIFT:" << m_id << "] Trace integrity verified (checksum 0x" << std::hex
                         << current_checksum << std::dec << ").\n";
         #endif
            }
            break;
         }
         ...
         case RecOtherIcacheVariable: {
            ...
            while (size_left > 0) {
               ...
               input->read(const_cast<char *>(reinterpret_cast<const char *>(&(icache[base_addr][offset]))),
                           read_amount);

#if VERBOSE_ICACHE
               std::cerr << __FUNCTION__ << ": Wrote " << read_amount << " bytes to 0x" << std::hex
                         << (void *)&(icache[base_addr][offset]) << std::dec << std::endl;
      ...
      printf("%016lx (%d) A%u %c%c %c%c\n", inst.sinst->addr, inst.sinst->size, inst.num_addresses,
             inst.is_branch ? 'B' : '.', inst.is_branch ? (inst.taken ? 'T' : '.') : '.', inst.is_predicate ? 'C' : '.',
...
bool Sift::Reader::AccessMemory(MemoryLockType lock_signal, MemoryOpType mem_op, uint64_t d_addr, uint8_t *data_buffer,
                                uint32_t data_size)
{
#if VERBOSE > 0
   if (mem_op == MemWrite)
